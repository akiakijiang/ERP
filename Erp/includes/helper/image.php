...
abstract class Helper_Image
{
    ...
    static function createFromFile($filename, $fileext)
    {
        ...
        {
        	throw new Exception('不支持的扩展名 imagecreateform : '.$fileext);
        }
	 ...
	 * 取得图片类型
	 * 
     * @return  int IMG_GIF | IMG_JPG | IMG_JPEG | IMG_PNG
	 */
	static function ext($resource)
	{
...
class Helper_ImageGD
{
    ...
    function crop($width, $height, $options = array())
    {
        ...
        imagecopyresized($dest,  $this->_handle, $dst_x, $dst_y, 0, 0, $dst_w, $dst_h, $full_w, $full_h);
        imagedestroy($this->_handle);
        $this->_handle = $dest;
     ...
     * 合并图片
     * 
     * @param Helper_ImageGD | Helper_ImageString $stamp
     * @param array 设置
     * 
     * @return Helper_ImageGD
     */
    function merge($stamp, $options = array())
    {        
    	if (!$stamp instanceof Helper_ImageGD && !$stamp instanceof Helper_String) {
    		throw new Exception('非法的来源图片');
    	}
    	...
    	imagecopymerge($this->_handle, $stamp->_handle, $dst_x, $dst_y, 0, 0, $src_w, $src_h, $options['pct']);
        return $this;
    }

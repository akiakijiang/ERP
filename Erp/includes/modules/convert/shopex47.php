 ...
 * 进行修改、使用和再发布。
 * ============================================================================
 * @author:    huiger <cyhkfc@hotmail.com>
 * @version:   v2.1
 * ---------------------------------------------
 * $Author: paulgao $
 * $Date: 2007-05-17 13:33:17 +0800 (星期四, 17 五月 2007) $
 * $Id: shopex47.php 8653 2007-05-17 05:33:17Z paulgao $
 */
...
class shopex47
{
    ...
    /* 构造函数 */
    function shopex47(&$sdb, $sprefix, $sroot, $scharset = 'UTF8', $iconv = NULL)
    {
        $this->sdb = $sdb;
        $this->sprefix = $sprefix;
        $this->sroot = $sroot;
        $this->troot = str_replace('/includes/modules/convert', '', str_replace('\\', '/', dirname(__FILE__)));
        $this->tdocroot = str_replace('/admin', '', dirname($_SERVER['PHP_SELF']));
    ...
    function process_cat()
    {
        ...
        /* 查询非底级分类id（因为shopex没有is_leaf字段） */
        $sql = "SELECT DISTINCT pid FROM ".$this->sprefix."mall_offer_pcat";
        $pid_list = $this->sdb->getCol($sql);
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $cat['cat_name']    = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['cat']));
            $cat['parent_id']   = $row['pid'];
            $cat['sort_order']  = $row['catord'];
            $cat['is_leaf']     = in_array($row['catid'], $pid_list) ? '0' : '1';
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $type['cat_name']   = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['cat_name']));
            $type['enabled']    = '1';
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $attr['attr_name']  = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['prop_name']));
    ...
    function process_brand()
    {
        ...
        {
            $brand = array(
                'brand_name' => $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['brand_name'])),
                'brand_desc' => '',
                'site_url' => $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['brand_site_url'])),
                'brand_logo' => $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['brand_logo']))
            );
    ...
    function process_goods()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $goods['goods_sn']      = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['bn']));
            $goods['goods_name']    = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['goods']));
            $goods['brand_id']      = trim($row['brand']) == '' ? '0' : $brand_list[$this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['brand']))];
            ...
            if ($row['tejia2'] == '1')
            {
                ...
                $goods['promote_start'] = date('Y-m-d');
                $goods['promote_end']   = date('Y-m-d', time() + 10 * 86400);
            }
            $goods['warn_number']   = $row['ifalarm'] == '1' ? $row['alarmnum'] : '0';
            $goods['goods_brief']   = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['intro']));
            $goods['goods_desc']    = str_replace('pictures/newsimg/', $this->tdocroot . '/images/upload/Image/', $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['memo'])));
            $goods['is_on_sale']    = $row['shop_iffb'];
            $goods['is_alone_sale'] = $row['onsale'];
            $goods['is_basic']      = $row['ifdiscreteness'];
            $goods['is_gift']       = '0';
            $goods['can_handsel']   = '1';
            ...
            while ($attr = $this->sdb->fetchRow($res1))
            {
                ...
                $goods_attr['attr_value']   = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($attr['prop_value']));
    ...
    function process_users()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $user_rank['rank_name']     = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['name']));
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $user['user_name']      = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['user']));
            $user['password']       = $row['password'];
            $user['question']       = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['pw_question']));
            $user['answer']         = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['pw_answer']));
            $user['sex']            = $row['sex'];
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $address['address_name']    = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['name']));
            $address['user_id']         = $row['memberid'];
            $address['consignee']       = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['name']));
            $address['email']           = $row['email'];
            $address['address']         = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['address']));
    ...
    function process_article()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $article['title']       = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['title']));
            $article['content']     = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['con']));
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $link['link_name']   = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['linktitle']));
    ...
    function process_order()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $order['order_time']        = date('Y-m-d H:i:s', $row['ordertime']);
            $order['consignee']         = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['name']));
            $order['address']           = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['addr']));
            $order['zipcode']           = $row['zip'];
            $order['tel']               = $row['tel'];
            $order['mobile']            = $row['mobile'];
            $order['email']             = $row['email'];
            $order['postscript']        = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['memo']));
            $order['shipping_name']     = is_null($row['tmethod']) ? ' ' : $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['tmethod']));
            $order['pay_name']          = is_null($row['payment']) ? ' ' : $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['payment']));
            $order['inv_payee']         = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['invoiceform']));
    ...
    function process_config()
    {
        ...
        $config = array();
        $config['shop_name']        = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['offer_name']));
        $config['shop_title']       = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['offer_shoptitle']));
        $config['shop_desc']        = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['offer_metadesc']));
        $config['shop_address']     = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['offer_addr']));
        $config['service_email']    = $row['offer_email'];
        $config['service_phone']    = $row['offer_tel'];
        $config['icp_number']       = $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['offer_certtext']));

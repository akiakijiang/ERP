...
function ip2province($ip) {
  static  $areatxt, $area;
  $areatxt = file_get_contents(ROOT_PATH . 'includes/ipdata/area.txt');
  $area = explode("\n",$areatxt);
  $str = iconv('gbk','UTF-8',convertip($ip));

  for($i = count($area)-1; $i > 0 ; $i--) {
    if(stripos($str, trim($area[$i])) !== false) {    
      break;
    }
  }
  return trim($area[$i]);
...
function convertip($ip) {
	...
	if($fd = @fopen(ROOT_PATH . 'includes/ipdata/ip.dat', 'rb')) {
		...
		$datadir = ROOT_PATH. 'includes/ipdata/';

...
$monitor_header = new MonitorHeader("自动售后收货情况监控页",array('service_id'));
...
function GenerateBackOrderInfo($service_id, $back_order_id, $facility_id){
	...
	$sql = "select service_order_goods_id, service_id, order_id, user_id, order_goods_id, approve_datetime, is_approved, amount
			from ecshop.service_order_goods where service_id = '{$service_id}'";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL(
	...
	// ecshop.ecs_indicate
	$sql = "select * from ecshop.ecs_indicate where service_id = '{$service_id}'";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL('出入库指示表[ecshop.ecs_indicate]', $sql, 'indicate_id');
	$back_info_for_generate[] = $result['monitor_info'];
	
	// ecshop.ecs_indicate_detail
	$sql = "select id.* from ecshop.ecs_indicate i
			inner join ecshop.ecs_indicate_detail id on i.indicate_id = id.indicate_id
			where i.service_id = '{$service_id}'";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL('出入库指示明细表[ecshop.ecs_indicate_detail]', $sql, 'indicate_detail_id');
	$back_info_for_generate[] = $result['monitor_info'];
	
	// ecshop.ecs_actual
	$sql = "select a.* from ecshop.ecs_indicate i 
			inner join ecshop.ecs_actual a on i.indicate_id = a.indicate_id
			where i.service_id = '{$service_id}'";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL('出入库实绩表[ecshop.ecs_actual]', $sql, 'actual_id');
	$back_info_for_generate[] = $result['monitor_info'];
	
	// ecshop.ecs_actual_detail
	$sql = "select ad.* from ecshop.ecs_indicate i
			inner join ecshop.ecs_actual_detail ad on i.indicate_id = ad.indicate_id
			where i.service_id = '{$service_id}'";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL('出入库实绩明细表[ecshop.ecs_actual_detail]', $sql, 'actual_detail_id');
	$back_info_for_generate[] = $result['monitor_info'];

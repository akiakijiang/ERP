...
if (isset($_GET['region_id']) && isset($_GET['order_number'])) {
	...
	$sql = "UPDATE ecshop.sync_jm_order_info SET district = (select " .
			" region_name from ecshop.ecs_region  where region_id ='$region_id'),transfer_status = 'NORMAL'" .
			" WHERE jm_order_number ='$order_number' ";
 	$abc = $db->query($sql);
 	header("Location:jm_order_list.php");
}			

if($act == "search"){
	$condition = get_condition();
	$sql = "SELECT jm_order_id, creation_time, pay_time, transfer_status, transfer_note, province, 
				city, county
			FROM ecshop.sync_jm_order_info
			WHERE".$condition." order by pay_time desc";
...
$smarty->display('weixin/jm_order_list.htm');
...
function get_condition(){
	...
	if( trim($_REQUEST['order_number']) != '' ){
		$condition .= " AND jm_order_id ='".trim($_REQUEST['order_number'])."'";
	}
	if( trim($_REQUEST['receiver']) != '' ){
		$condition .= " AND receiver_name = '".trim($_REQUEST['receiver'])."'";
	}
	if( trim($_REQUEST['transfer_status']) != 'ALL' ){
		$condition .= " AND transfer_status = '".trim($_REQUEST['transfer_status'])."'";
	}
	if($startTime && $endTime){
		$condition .= " AND pay_time > '".trim($_REQUEST['startDate'])."' and pay_time <= '".trim($_REQUEST['endDate'])."'";
	}

...
//            pp($json);             
 ...
 if ('create_item' == $act) {	
	...
	$ret_orginal_id = $_POST['ret_original_id'];
 	...
 	foreach($ret_orginal_id as $key=>$item){		 		 		
        ...
        sys_msg("{$sys_message}",0,array(array('href'=>'./generate_batch_sn_gt.php')),true);
 	}
 }   
 ...
 if ('export_goods_storage' == $act) {    
    ...
    unset($ret_items); 
	export_batch_sn_return_list_excel($excel_result);
	exit();
 }
 ...
 $smarty->display('oukooext/generate_batch_sn_gt.htm');
 QLog::log('out');
?>

 ...
 */
 ...
 if ('cancle' == $act && !empty($supRetReqId)) { 
    // 直接更新状态
    $status = 'CANCELLATION' ;
    update_supplier_return_request_status($supRetReqId, $status);
    ...
    if ( empty($batch_gt_id_excel)){
     ...
     if(!empty($used_batch_gt_sn) || !empty($request_id_excel) ){  // 如果输入了 batch_gt_sn 号 
	     ...
	     $request = new stdClass();
	          ...
	          $request -> facilityId = $_SESSION['facility_id'];
		 ...
		 if($_SESSION['facility_id']){
		 	$countSQl .= " and facility_id in (".$_SESSION['facility_id'].")";
		 } 
	     ...
	     $sql="select gt.supplier_return_gt_sn ,gt.supplier_return_id ,oi.order_id
			     	      ...
			     	      where  ".Helper::db_create_in('gt.supplier_return_id',$supplierReturnRequestIds);
		 ...
		 foreach($barcodeResult as $value){
			  ...
			  // 获取 barcode 对应的 spec 
		 $sql = "select spec, barcode, item_number from ecshop.brand_zhongliang_product WHERE ".Helper::db_create_in('barcode',$barcodes);
		 $specResult = $db->getAll($sql);
			  // 组织 spec  数据   barcode => spec 
		 $spec_format = array();
		 $item_number_format = array();
		 foreach($specResult as $value){
			 	$spec_format[$value['barcode']] = $value['spec'];
			 	$item_number_format[$value['barcode']] = $value['item_number'];
		 }	 	 
			//根据 supplierReturnRequestId 获取batch_sn	 
		 $batch_sn_format = array();	 
		 $sql = "select batch_sn,supplier_return_id from romeo.supplier_return_request where ".Helper::db_create_in('supplier_return_id',$supplierReturnRequestIds);
	     ...
	     $sql = "select ifnull(gm.batch_gt_sn,'') as batch_gt_sn, gm.order_id ,  
	     		 boi.postscript as po_order_id , boi.outer_remain_sn as sap_order_id , boi.outer_out_sn as sap_out_order_id
				from  ecshop.ecs_batch_gt_mapping gm  LEFT JOIN ecshop.ecs_batch_gt_info boi on gm.batch_gt_sn = boi.batch_gt_sn WHERE ".Helper::db_create_in('gm.order_id',$order_id_list);
	    ...
	    foreach($batch_gt_sn_result as $value){
	     		$batch_gt_sn_format[$value['order_id']]['batch_gt_sn'] = $value['batch_gt_sn'];
	     		$batch_gt_sn_format[$value['order_id']]['po_order_id'] = $value['po_order_id'];
	     		$batch_gt_sn_format[$value['order_id']]['sap_order_id'] = $value['sap_order_id'];
	     		$batch_gt_sn_format[$value['order_id']]['sap_out_order_id'] = $value['sap_out_order_id'];
	    } 
	   ...
	   foreach($requestList as &$item ){
	            ...
	            // 加入物料编码
	            $item['item_number'] =  $item_number_format[$item['barcode']];
	            ...
	            // 批次号 
	            $item['batch_gt_sn'] =   $batch_gt_sn_format[$tmp_order_id]['batch_gt_sn'];
	            $item['po_order_id'] =   $batch_gt_sn_format[$tmp_order_id]['po_order_id'];
	            $item['sap_order_id'] =   $batch_gt_sn_format[$tmp_order_id]['sap_order_id'];
	            $item['sap_out_order_id'] =   $batch_gt_sn_format[$tmp_order_id]['sap_out_order_id'];
 ...
 if(isset($csv) && !empty($csv)){
    ...
    header("Content-Disposition:filename=" . iconv("UTF-8","GB18030","{$party_name}批量B2B出库详情导出") . ".csv");
    ob_start();
    $header_str = iconv("UTF-8",'GB18030',"PO订单号,SAP订单号,SAP出库单号,乐其系统出库单号,物料编码,商品名称,商品条码,出库数量,出库箱数\n");
    $file_str = "";
    foreach($resultList as $picihao => $oneList){
    	...
    	$file_str .= str_replace(","," ",$req['po_order_id']).",";
        $file_str .= str_replace(","," ",$req['sap_order_id']).",";
    	$file_str .= str_replace(","," ",$req['sap_out_order_id']).",";
    	$file_str .= '="'.str_replace(",","",$picihao).'"'.",";
    	$file_str .= str_replace(","," ",$req['item_number']).",";
 ...
 $smarty->display('supplier_return/zhongliang_return_goods_request_list.htm');
  ...
  *    batch_gt_sn => array( is_batch,data=>array(),count,po_order_id,sap_order_id,sap_out_order_id) 
  */
 function formateDataByBatchGtsn($resultList){
	...
	foreach($resultList as $item) {
		if(empty($item['batch_gt_sn'])) {
	     		...
	     		$format_datas[$batch_order_sn]['po_order_id'] = $item['po_order_id'];
	     		$format_datas[$batch_order_sn]['sap_order_id'] = $item['sap_order_id'];
	     		$format_datas[$batch_order_sn]['sap_out_order_id'] = $item['sap_out_order_id'];
	    }
 ...
 class BatchGt{
	...
	public static function get_supplier_return_list($batch_gt_sn,$fields =null,&$ref_fields) {
		...
		ifnull(gs.barcode,g.barcode) as barcode,og.goods_number,zp.spec,zp.item_number,

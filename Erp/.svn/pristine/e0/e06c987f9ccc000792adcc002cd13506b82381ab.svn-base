...
if($_REQUEST['request'] == 'search'){
	...
	$sql = "
		select
			...
			if(ba.asn_status='FULFILLED',ba.last_updated_stamp,'') wms_fulfill_time,
			if(o.in_time=0,'',o.in_time) erp_fulfill_time
		from
			ecshop.ecs_batch_order_info o
			left join ecshop.express_best_indicate bi on o.batch_order_id=bi.order_id
			left join ecshop.express_best_actual  ba on o.batch_order_id=ba.order_id
		where
			o.party_id=65625 and o.is_cancelled='N' {$condition}
	";
	...
	$pager = setPager($count, $size, $page,"zhongliang_purchase_order_list.php?request=search".getReqString());
...
$smarty->display("taobao/zhongliang_purchase_order_list.htm");
...
function get_condition(){
	...
	if (isset($_REQUEST['batch_order_id']) && trim($_REQUEST['batch_order_id']) != '') {
		$batch_order_id = mysql_real_escape_string(trim($_REQUEST['batch_order_id']));
        $condition .= " AND o.batch_order_id ={$batch_order_id}";
    }

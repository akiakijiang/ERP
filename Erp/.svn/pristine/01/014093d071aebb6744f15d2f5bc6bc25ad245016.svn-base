...
class Helper_Uploader_File
{
	...
	function isValid($allowed_types = null, $max_size = null)
	{
		...
		if ($allowed_types)
        {
            ...
            {
                $allowed_types = array_filter(array_map('trim', explode(',', $allowed_types)), 'strlen');   
            }
            ...
            //读取文件名
            $filename = strtolower(preg_replace('/^.+[\\\\\\/]/', '', $this->filename()));
            //$filename = strtolower(basename($this->filename()));
            $extnames = !is_array($filename)
                ? array_filter(array_map('trim', explode('.', $filename)), 'strlen')
                : $filename;

...
$sql = "SELECT * FROM {$ecs->table('order_info')} WHERE order_id = '{$order_id}' AND " . party_sql('party_id');
$order = $db->getRow($sql, true);
if ($order == null) {
    die("非法输入");
}

//日期录入方式 ljzhou 2013.05.27
...
$url = "sn_inputV2.php?order_id={$order_id}";
...
$smarty->display('oukooext/sn_inputV2.htm');

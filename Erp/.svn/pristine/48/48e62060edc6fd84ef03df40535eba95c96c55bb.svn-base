 ...
 */
class ErpSyncWeixinRTMCommand extends CConsoleCommand
{	
	/**
	 * 同步微信人头马订单
	 */
	public function actionSyncWeixinOrder($appkey=null,$hours=6,$pageNo=0)
	{
		...
		echo("[".date('c')."] "." sync rtm weixin order start endDate:".$endDate."\n");
	
		// 远程服务
		$client=Yii::app()->getComponent('erpsync')->SyncWeixinRTMService;
		foreach($this->getWeixinShopList() as $taobaoShop)
		{
			...
			echo("[".date('c')."] ".$taobaoShop['nick']." SyncWeixinRTMOrder start \n");
			$start = microtime(true);
			// 同步生成订单
			try
			{
				$request=array("applicationKey"=>$taobaoShop['application_key'],"hours"=>$hours,"endDate"=>$endDate,"pageNo"=>$pageNo);
				print_r($request);
				$response=$client->SyncWeixinRTMOrder($request);
				print_r($response);
			}
			catch(Exception $e)
			{
	...
	public function actionWeixinOrderTransfer($appkey=null,$days=1)
	{
		...
		foreach($this->getWeixinShopList() as $taobaoShop)
		{
			...
			echo("[".date('c')."] ".$taobaoShop['nick']." WeixinRTMOrderTranfer start \n");
			$start = microtime(true);
			// 同步生成订单
			try
			{
				...
				print_r($request);
				$response=$client->WeixinRTMOrderTransfer($request);
				print_r($response);
			}
			catch(Exception $e)
			{
	...
	public function actionSyncDeliverySend($appkey=null)
	{
		...
		foreach($this->getWeixinShopList() as $taobaoShop)
		{
			...
			echo("[".date('c')."] ".$taobaoShop['nick']." SyncWeixinRTMSendDelivery start \n");
			$start = microtime(true);
			// 同步生成订单
			try
			{
	 ...
	 * 
	 */
	public function actionSyncWeixinStock($appkey=null)
	{
		// 不启用商品同步的列表
		$exclude_list=array
		(
		...
		foreach($this->getWeixinShopList() as $taobaoShop)
		{
			...
			try
			{
				$request=array("applicationKey"=>$taobaoShop['application_key']);
				print_r($request);
				$response=$client->SyncWeixinRTMStock($request);
				print_r($response);
			}
			catch(Exception $e)
			{
	...
	protected function getWeixinShopList()
	{
		...
		if(!isset($list))
		{
			$sql="select * from taobao_shop_conf where status='OK' and shop_type = 'weixinjf' ";

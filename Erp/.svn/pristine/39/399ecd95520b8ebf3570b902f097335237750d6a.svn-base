...
$sql = "
		SELECT
	        ...
	        og.rec_id,og.goods_name, og.goods_number, og.customized, og.goods_id, og.style_id, g.is_maintain_warranty,g.is_maintain_batch_sn, gs.internal_sku, if(gs.barcode is NULL or gs.barcode = '',g.barcode,gs.barcode) as barcode,
	        boi.order_type, boi.provider_id, p.provider_name, boi.purchaser, 
			boi.remark, 
	        o.facility_id, f.facility_name, boi.batch_order_sn
		FROM 
...
if (! empty ( $search_orders )) {
	...
	$search_order_goods = $db->getAllRefBy ( $sql, array ('rec_id' ), $refs_value_count_1, $refs_count_1 );
	...
	foreach ( $search_orders as $key => $order ) {
		...
		if($sort_method == 'bigger2zero' && $search_orders [$key] ['not_in_count']==0) {unset($search_orders[$key]);}
	}
...
$url = "batch_or_edit_in_storageV2.php?batch_order_id={$batch_order_id}";
...
$smarty->assign('info', $info);
$smarty->display ( 'oukooext/batch_or_edit_in_storageV2.htm' );

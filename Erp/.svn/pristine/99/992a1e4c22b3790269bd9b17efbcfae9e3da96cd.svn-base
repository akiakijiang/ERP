...
if($act == 'change'){
	...
	if(!$result){
		...
		$record_sql = "INSERT INTO ecshop.risky_actions (admin_user, type, timestamp, php_location, form_name, `sql`, comment) " .
            			"VALUES ('{$_SESSION['admin_name']}', 'update', NOW(), 'jd_items_update.php', 'jd_items_update', '".mysql_real_escape_string($sql)."', '{$_POST['comment']}')";
        $db->query($record_sql);
		$result = "Success";
	}else{
		$result = "FAIL";
	...
	$sql1 = "SELECT * FROM ecshop.sync_jd_items_sku WHERE sku_id = '{$sku_id}'";
	$result = "数据更新成功";
	if( $sku_id != '' && $db->getRow($sql1) ){
		$sql = "UPDATE ecshop.sync_jd_items_sku SET {$field} = '{$number}' WHERE sku_id = '{$sku_id}'";
		if(!$db->query($sql)){
			$result = "数据更新失败";
		}
	}else{
	...
	$result = 'fail';
	if($sku_id != '' && $db->getRow($sql1)){
		$sql = "UPDATE ecshop.sync_jd_items_sku SET is_sync = $is_sync WHERE sku_id = '{$sku_id}'";
//		Qlog::log($sql);
		$db->query($sql);
	}
	...
	if($rowset){ 
		foreach($rowset as $row) {
			...
			$jd_inventory_ratio = $row['jd_inventory_ratio'];
			$taobao_inventory_ratio = $row['taobao_inventory_ratio'];
			$amazon_inventory_ratio = $row['amazon_inventory_ratio'];
			if(!(is_numeric($jd_inventory_ratio) && $jd_inventory_ratio>=0 && $jd_inventory_ratio<=1)){
			...
			$sql1 = "update ecshop.sync_jd_items_sku set inventory_ratio={$jd_inventory_ratio} where outer_id='{$outer_id}' AND party_id = {$party_id}";
...
$application_list = get_jd_shop_nicks();
...
if($_REQUEST['request'] == 'search'){
	...
	$sql = "select
				sc.nick,js.outer_id,js.sku_id,js.goods_id,js.style_id,ji.title,
				...
				js.inventory_ratio,
				js.quantity,js.reserve_number,js.reserve_ratio,js.warn_number,IF(js.is_sync = 0,'不同步','同步') as is_sync, js.sku_status, js.last_update_timestamp
			from 
				ecshop.sync_jd_items_sku  js
				left join ecshop.sync_jd_items ji on js.ware_id=ji.ware_id
	...
	//Qlog::log($sql1);
	//var_dump($sql1);
	$jd_items_list = $db->getAll($sql1);
	// print_r($jd_items_list);die();
	$sql2 = "SELECT COUNT(*) FROM(".$sql.") as countItems";
			
	$count = $db->getOne($sql2);
	$pager = setPager($count, $size, $page,"jd_items_update.php?request=search&goods_name=".trim($_REQUEST['goods_name'])."&outer_id=".$_REQUEST['outer_id'].
	...
	$smarty->assign('jd_items_list',$jd_items_list);
}

$smarty->display("jd/jd_items_update.htm");

/**
 * 取得淘宝店铺信息
 * 
 */
function get_jd_shop_nicks() {
    $application_list = get_jd_shop_list(); //取得taobao店铺信息
...
function get_jd_shop_list() {
    global $db;
    $sql = "SELECT nick,application_key FROM ecshop.taobao_shop_conf WHERE shop_type in ('360buy', '360buy_overseas') and party_id = '".$_SESSION['party_id']."'";
...
function get_code($outer_id){
	...
	$sql="SELECT goods_id,style_id FROM ecshop.sync_jd_items_sku WHERE outer_id='{$outer_id}'";
	$re= $db->getAll($sql);
	//var_dump($re);

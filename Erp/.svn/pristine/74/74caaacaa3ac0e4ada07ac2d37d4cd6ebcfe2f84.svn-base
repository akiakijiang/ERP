...
if ($act == "update") {  // 验货入库、拒绝入库操作
	...
	//sleep(5);
	// 获取service记录
	$sql = "SELECT * FROM ecshop.service WHERE service_id = '{$service_id}' and facility_id not in ('{$outShip_facility}') "; 
	...
	if ($inner_check == "pass") { // 验货通过
		...
		$sql = " 
		 SELECT sum(wait_inv_in) from (
			select og.rec_id,(og.goods_number- sum(IFNULL(iid.QUANTITY_ON_HAND_DIFF,0) )) as wait_inv_in from ecshop.service s  
	    ...
	    if($serviceCount != $countOfGoods){
	    	sys_msg("对不起，提交的数据错误！请重新提交 商品数量小于需要扫描的数量", 1);
	    }
	    ...
	    if ($goodsNum > 0) {
	         ...
	         if($serviceCount == 0 ){ // 已全部入库 
	            ...
	            POSTSALE_CACHE_updateService(null,180,$service_id);
	         	...
	         	sys_msg("对不起，入库操作失败！有部分商品未入库。请重试,重试失败,请联系ERP组", 1);
	         }  
        } else {
	    	sys_msg("对不起，入库操作失败！重试失败,请联系ERP组", 1);
        }
...
elseif ($act == 'edit_service_return') {
    ...
    $sql = "UPDATE service SET backfee_paiedby = '{$_POST['backfee_paiedby']}' WHERE service_id = '{$service_id}' and facility_id not in ('{$outShip_facility}')  LIMIT 1 ";
    $db->query($sql);
...
elseif ($act == 'remark') {
    extract($_POST);
    $sql = "SELECT * FROM service WHERE service_id = '{$service_id}' and facility_id not in ('{$outShip_facility}')  ";
    $service = $db->getRow($sql);
...
else
{
    ...
    $sql = " SELECT *, s.service_id, s.order_id as service_order_id, s.back_order_id,s.facility_id, o.facility_id as origin_facility_id 
    		FROM service s
         INNER JOIN {$ecs->table('order_info')} o ON s.order_id = o.order_id
         WHERE s.service_type IN ('" .SERVICE_TYPE_CHANGE . "', '" . SERVICE_TYPE_BACK .
         	"') and s.facility_id not in ('{$outShip_facility}')
    ...
    // 获取待处理的售后服务数量
    $date = date('Y-m-d', strtotime("-1 months", time()));//查询近一个月还未处理的售后
    $sql_c_pending = " SELECT COUNT(*) FROM service s
           ...
           WHERE s.service_type IN ('" .SERVICE_TYPE_CHANGE . "', '" . SERVICE_TYPE_BACK ."')
            	 ...
            	 AND s.back_shipping_status IN (0,5) 
            	  and s.facility_id not in ('{$outShip_facility}')
            	  AND s.apply_datetime >= '{$date}'
           ";
    $count_pending = $db->getOne($sql_c_pending);
    $count_var['back_shipping_status_0'] = $count_pending;
 // QLog::log("back_goods_services:".$sql);
 // QLog::log("count:".$sql_c);
 // QLog::log("count_pending:".$sql_c_pending);
    ...
    foreach ($back_goods_services as $service_key => $service) {
        ...
        foreach($back_goods_list as $goods_key => $goods){
        	...
        	if($is_serial == 'SERIALIZED'){  // 串号控制商品需要根据不同串号进行统计
        		...
        		$back_goods_list[$goods_key]['amount_request'] = $back_goods_list[$goods_key]['amount'];
        		$back_goods_list[$goods_key]['amount'] = $back_goods_list[$goods_key]['amount'] - $new_number - $old_number - $defective_number - $unknown_number;   
        		if($new_number > 0  ){
        			$new_old_status .= "<strong>全新:</strong><span style='color:red;'>".$new_number."</span>个,"; 
        		}else{
        			$new_old_status .= "<strong>全新:</strong>".$new_number."个,";
        		}

        		if($old_number > 0){
        			$new_old_status .="<strong>二手:</strong><span style='color:red;'>".$old_number."</span>个,"; 
        		}else{
                    $new_old_status .="<strong>二手:</strong>".$old_number."个,"; 
        		}
        		if( $defective_number > 0){
        			$new_old_status .= "<strong>废弃:</strong><span style='color:red;'>".$defective_number."</span>个,"; 
        		}else{
        			$new_old_status .= "<strong>废弃:</strong> ".$defective_number."个,";  
        		}

        		if($unknown_number > 0){
        			$new_old_status .= "<strong>未知:</strong><span style='color:red;'>".$unknown_number."</span>个"; 
        		}else{
        			$new_old_status .= "<strong>未知:</strong>".$unknown_number."个";  
        		}
        ...
        $back_goods_services[$service_key]['facility_name'] = facility_mapping($back_goods_services[$service_key]['facility_id']);
        $back_goods_services[$service_key]['origin_facility_name'] = facility_mapping($back_goods_services[$service_key]['origin_facility_id']);;
	            ...
	            foreach ($rma_tracks as $track) {
	                ...
	                if ($track->serviceId == $service['service_id']) {
	                    ...
	                    if ($track->trackAttribute == null)  {
	                        $back_goods_services[$service_key]['can_in_storage'] = false;
	                    }
	                }
	            }
	        }
...
function getCondition() {
    ...
    if ($_REQUEST['act'] == 'search') {
    	...
    	$date = date('Y-m-d', strtotime("-1 months", time()));
    	$condition .= " AND s.apply_datetime >= '{$date}' ";
    }

...
if (! empty ( $search_orders )) {
	$in_order_ids = db_create_in ( $refs_value_order ['rec_id'], 'rec_id' );

    // 查询每个订单的已入库数
    $sql = "
        SELECT
            og.rec_id,og.order_id, ifnull(sum(iid.quantity_on_hand_diff),0) AS in_count
        FROM
            ecs_order_goods og
...
$url = "batch_edit_in_storageV2.php?batch_order_id={$batch_order_id}";
...
$smarty->assign('info', $info);
$smarty->display ( 'oukooext/batch_edit_in_storageV2.htm' );

...
try {
	do {
		...
		if (count ( $rec_ids ) != $check_number || count ( $batch_order_sns ) != $check_number || count ( $goods_item_types ) != $check_number || count ( $is_maintain_warrantys ) != $check_number || count ( $goods_barcodes ) != $check_number) {
		...
		for($i = 0; $i < $check_number; $i ++) {
			try {
				...
				for($j = 0; $j < count ( $validitys ); $j ++) {
					...
					$info = one_order_purchase_accept_and_location_transaction_V2($order_id,$location_barcode,$goods_barcode,$serial_number,
	                                                           $input_number,$validity,$rec_id, $validity_type='start_validity');
...
$back = add_param_in_url ( "batch_in_storageV2.php?", 'info', $back_message . " 耗时" .$cost_time);

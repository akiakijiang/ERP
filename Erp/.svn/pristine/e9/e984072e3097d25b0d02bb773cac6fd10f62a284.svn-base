...
//验证权限
//admin_priv('suning_items_update');
...
array ('批量导入库存比例' => 
         array ('outer_id' => '商家编码', 
                'suning_inventory_ratio' => '苏宁海外购库存比例') );
...
if($act == 'change'){
	...
	if(!$result){
		...
		$record_sql = "INSERT INTO ecshop.risky_actions (admin_user, type, timestamp, php_location, form_name, `sql`, comment) " .
            			"VALUES ('{$_SESSION['admin_name']}', 'update', NOW(), 'suning_items_update.php', 'suning_items_update', '".mysql_real_escape_string($sql)."', '{$_POST['comment']}')";
        $db->query($record_sql);
		$result = "Success";
	}else{
		$result = "FAIL";
	...
	//$sql1 = "SELECT * FROM ecshop.sync_suning_items_sku WHERE product_code = '{$product_code}'";
	$result = "数据更新成功";
	if( $product_code != '' ){
		$sql = "UPDATE ecshop.sync_suning_items_sku SET {$field} = '{$number}' WHERE product_code = '{$product_code}'";
	...
	$sql1 = "SELECT * FROM ecshop.sync_suning_items_sku WHERE product_code = '{$product_code}'";
	$result = 'fail';
	if($product_code != '' && $db->getRow($sql1)){
		$sql = "UPDATE ecshop.sync_suning_items_sku SET is_sync = $is_sync WHERE product_code = '{$product_code}'";
		$db->query($sql);
	}
	...
	if($rowset){ 
		foreach($rowset as $row) {
			...
			$suning_inventory_ratio = $row['suning_inventory_ratio'];
			if(!(is_numeric($suning_inventory_ratio) && $suning_inventory_ratio>=0 && $suning_inventory_ratio<=1)){
					$message .= "第".($key+1)."行的{$outer_id}苏宁海外购库存比例不在0到1之间 <br/>";
					continue;
			}
			

			$sql = "update ecshop.sync_suning_items_sku set inventory_ratio={$suning_inventory_ratio} where item_code='{$outer_id}' AND party_id = {$party_id}";
...
$application_list = get_suning_shop_nicks();
...
if($_REQUEST['request'] == 'search'){
	...
	$sql = "select
				sc.nick,js.item_code,js.product_code,js.goods_id,js.style_id,js.product_name,
			...
			from 
				ecshop.sync_suning_items_sku  js
				left join ecshop.sync_suning_items ji on js.item_sku_code=ji.item_code
				...
				LEFT JOIN ecshop.distribution_group_goods dg ON js.item_code = dg.code
				LEFT JOIN ecshop.taobao_shop_conf sc on js.application_key = sc.application_key
			where
				1 ".$condition.
		   " group by js.product_code" . $outer_Conditon;
		
	$sql1 = $sql.$limit.$offset;
	//Qlog::log($sql1);
	//var_dump($sql1);
	$suning_items_list = $db->getAll($sql1);
	// print_r($jd_items_list);die();
	$sql2 = "SELECT COUNT(*) FROM(".$sql.") as countItems";
			
	$count = $db->getOne($sql2);
	$pager = setPager($count, $size, $page,"suning_items_update.php?request=search&goods_name=".trim($_REQUEST['goods_name'])."&outer_id=".$_REQUEST['outer_id'].
	...
	$smarty->assign('suning_items_list',$suning_items_list);
}

$smarty->display("jd/suning_items_update.htm");

/**
 * 取得淘宝店铺信息
 * 
 */
function get_suning_shop_nicks() {
    $application_list = get_suning_shop_list(); //取得taobao店铺信息
...
function get_suning_shop_list() {
    global $db;
    $sql = "SELECT nick,application_key FROM ecshop.taobao_shop_conf WHERE shop_type = 'suning' and party_id = '".$_SESSION['party_id']."'";
...
function get_code($outer_id){
	...
	$sql="SELECT goods_id,style_id FROM ecshop.sync_suning_items_sku WHERE item_code='{$outer_id}'";
	$re= $db->getAll($sql);
	//var_dump($re);

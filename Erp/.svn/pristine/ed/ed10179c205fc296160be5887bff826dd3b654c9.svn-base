 ...
 * @copyright ouku.com
 */
die("该页面已经被新页面替换，请及时更新个人设置，或直接使用新页面操作");
define('IN_ECS', true);
...
switch ($request)
{
    case 'ajax':
        // ajax请求, 查询是否有更新
        $result = refund_get_all_by_conditions($conditions, true);
    ...
    default:
        // 列表数据
        $list = refund_get_all_by_conditions($conditions);
}
...
else
{
    ...
    $smarty->display('oukooext/refund_list.htm');
}
...
function _refund_list_conditions(&$conditions)
{
    ...
    {
        $order_id = $db->getOne("SELECT order_id FROM {$ecs->table('order_info')} WHERE order_sn = '{$conditions['order_id']}'", true);

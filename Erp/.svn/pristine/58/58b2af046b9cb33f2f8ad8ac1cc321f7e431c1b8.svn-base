...
class ScnIndicateCommand extends CConsoleCommand {
    ...
    protected function getMaster() {
 	...
 	public function actionErpGoodsToNewBalance (){
		...
		$sql = "
			...
			where g.goods_party_id = 65611 and if(gs.style_id is null, g.barcode, gs.barcode) is not null 
		...
		if (!empty($goods_list)) {
			...
			foreach ($goods_list as $key => $goods_item) {
				...
				$goods_info[] = array("IMDN", "{$goods_item['barcode']}", "NB-MX", "{$goods_item['barcode']}", "{$goods_name}", "{$color_name}", "{$goods_item['size']}",
					"", "", "", "", $is_active);
			}
 	...
 	public function actionInventoryInIndicate(){
		...
		if (!empty($res->return->result)) {
			...
			foreach ($order_list as $key => $item) {
				...
				//入库头指示
				$txt_info[] = array("RCHDRDNNEW", "ILC-SH", "{$item->indicateId}", "{$item->indicateId}", "NB-MX", "名鞋入库", "");
				//商品明细
				if (!empty($item->indicateDetailList)) {
					...
					foreach ($detail as $goods_item) {
						...
						$txt_info[] = array("RCDTLDNNEW", "{$item->indicateId}", "{$goods_item->indicateDetailId}", "NB-MX", "{$goods_item->barcode}", "", 
										"{$goods_item->goodsNumber}", "{$color}", "{$goods_item->styleName}", "", "{$goods_item->indicateDetailId}", "名鞋入库", "{$goods_item->indicateDetailId}");
					} 
				}
			}
		...
		if (!empty($res->return->result)) {
			...
			foreach ($order_list as $key => $item) {
				...
				$txt_info[] = array("SHHDRDNNEW", "ILC-SH", "{$item->indicateId}", "{$item->indicateId}",
				 "NB-MX", "{$item->taobaoUserId}", "{$item->goodsAmount}", "{$item->shippingFee}", "", "",
				...
				if (!empty($item->indicateDetailList)) {
					...
					foreach ($detail as $goods_item) {
						...
						//样式名称 size
						$txt_info[] = array("SHDTLDNNEW", "{$item->indicateId}", "{$goods_item->indicateDetailId}", "{$goods_item->indicateDetailId}", "NB-MX", 
		...
		if (!empty($res->return->result)) {
			...
			foreach ($order_list as $key => $item) {
				...
				$txt_info[] = array("RCHDRRTNEW", "ILC-SH", "{$item->indicateId}", "{$item->indicateId}",
				 "NB-MX", "{$item->taobaoUserId}", "", "", "{$consignee}", "{$item->provinceName}",
				 "{$item->cityName}", "{$item->districtName}", "{$item->zipcode}", "{$address}", "", "", 
			     "", "{$item->tel}", "{$item->mobile}", "", "{$item->taobaoOrderSn}", "名鞋退货", "",
				); 
				if (!empty($item->indicateDetailList)) {
					$detail = array();
					if (!is_array($item->indicateDetailList->IndicateDetail)){
					...
					foreach ($detail as $goods_item) {
						...
						//样式名称 size
						$txt_info[] = array("RCDTLDNNEW", "{$item->indicateId}", "{$goods_item->indicateDetailId}", "NB-MX", "{$goods_item->barcode}", 
							"0", "{$goods_item->goodsNumber}","{$color}", "{$goods_item->styleName}", "", "{$goods_item->indicateDetailId}", "名鞋退货", "{$goods_item->indicateDetailId}");
					}
				}
			}
			$upload_result = $this->IndicationFile($file_name, $txt_info);
			...
			foreach ($order_list as $key => $item) {
				...
				if ($flag) {
					$txt_info[] = array("SHHDRRTNEW", "ILC-SH", "{$item->indicateId}", "{$item->indicateId}",
					 "NB-MX", TEMP_CONSIGNEE, "CN", TEMP_PROVINCE, 
					 TEMP_CITY, TEMP_DISTRICT, "{$item->zipcode}", TEMP_ADDRESS, "", "", 
				     TEMP_SHIPPING, TEMP_SHIPPING, TEMP_MOBILE, "{$order_time}", "0", "", "",  "Y", "名鞋返厂",
					); 
					$flag = false;
				...
				if (!empty($item->indicateDetailList)) {
					...
					foreach ($detail as $goods_item) {
						//样式名称 size
						$txt_info[] = array("SHDTLRTNEW", "{$item->indicateId}", "{$goods_item->indicateDetailId}", "{$goods_item->indicateDetailId}", "NB-MX", 
 	...
 	public function actionActual() {
		...
		if ($conn_ftp) {
			global $nb_scn_ftp_performance;
			$file_name_list = ftp_nlist($conn_ftp, $nb_scn_ftp_performance);
			foreach ($file_name_list as $item) {
				$name = basename($item);
				if (strpos($name, ".")) {
					$down_result = $this->downloadFile($conn_ftp, $name, $nb_scn_ftp_performance);
					if ($down_result) {
 	...
 	protected function readTxtFile($ftp_path_name){
		$start = time();
		global $nb_scn_ftp_performance;
		$ftp_path_name = ROOT_PATH."../newbalance/".$nb_scn_ftp_performance . "/". $ftp_path_name;
		$order_info = array();
 	...
 	protected function IndicationFile ($file_name, $order_list) {
		...
		$write_result = $this->writeFile(ROOT_PATH."../newbalance/indicates_data_scn/".$file_name, $order_list);
		$write_time = time();
		...
		if ($write_result){
			...
			if ($conn_id !== null) {
				$result = $this->uploadFile($conn_id, $file_name, ROOT_PATH."../newbalance/indicates_data_scn/".$file_name);
				ftp_close($conn_id);  
			}
		}
		return $result;
 	...
 	protected function uploadFile ($conn_id, $ftp_file_name, $file_name) {
		global $nb_scn_ftp_indicates;
		$ftp_dir_file = $nb_scn_ftp_indicates."/".$ftp_file_name;
		$result = false;
		$start = time();
 	...
 	public function actionInventoryInActual() {
		...
		foreach ($orders as $order) {
			...
			foreach($purchase_orders as $purchase_order) {
				if (! check_batch_in_storage_status($purchase_order['order_id'])) {
					$is_in_storage = false;
					break;
				}
			}
 	...
 	public function actionInventoryReturnActual() {
		...
		foreach ($orders as $order) {
			...
			if ($info['res'] == 'success') {
				...
				if ($back_order_id && check_batch_in_storage_status($back_order_id)) {

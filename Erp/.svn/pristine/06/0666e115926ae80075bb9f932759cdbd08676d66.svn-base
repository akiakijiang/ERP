 ...
 */
class ErpSyncSfhkCommand extends CConsoleCommand
{
   ...
   /**
     * 同步乐其微商城订单（by hzhang1 2015-07-16）
    ...
    public function actionSyncOrder($appkey=null,$hours=6)
    {
    	...
    	// 远程服务
    	$client=Yii::app()->getComponent('erpsync')->SyncSfhkService;
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." syncSfhkOrder start \n");
    		...
    		try
    		{
    			...
    			$response=$client->SyncSfhkOrder($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     /**
     * 转换乐其订单（by hzhang1 2015-06-30）
     * @param string $appkey 执行店铺的应用编号
     * @param int hours 订单同步时间距离当前时间的小时数
     */
     public function actionSfhkOrderTransfer($appkey=null,$days=1)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SfhkOrderTransfer start \n");
			$start = microtime(true);
    		...
    		try
    		{
				...
				$response=$client->SfhkOrderTransfer($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     /**
     * 同步发货（by hzhang1 2015-06-30）
     * @param string $appkey 执行店铺的应用编号
     */
     public function actionSyncSfhkOrderSendDelivery($appkey=null)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncSfhkOrderSendDelivery start \n");
			$start = microtime(true);
    		...
    		try
    		{
				...
				$response=$client->SyncSfhkOrderSendDelivery($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     /**
     * 收货确认（by hzhang1 2015-06-30）
     * @param string $appkey 执行店铺的应用编号
     * @param float hours 
     */
     public function actionSyncSfhkOrderFinished($appkey=null,$hours=1)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncSfhkOrderFinished start \n");
			$start = microtime(true);
    		...
    		{
    			$request=array("applicationKey"=>$taobaoShop['application_key'],"hours"=>$hours);
				$response=$client->SyncSfhkOrderFinished($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
    ...
    protected function getTaobaoShopList()
    {
    	...
    	if(!isset($list))
    	{
    		$sql="select * from taobao_shop_conf where status='OK' and shop_type = 'sfhk'";

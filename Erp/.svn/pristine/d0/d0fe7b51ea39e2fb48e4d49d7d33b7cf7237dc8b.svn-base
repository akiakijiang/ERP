...
$outShip_facility = implode("','", $outShip_facility_list);
 ...
 if ('search' == $act) {
	...
	if($input_facility_id){
		...
		if($_SESSION['facility_id']){
			$session_facility_id = str_replace(",","','",$_SESSION['facility_id']);
		 	$count_cond .= " and rt.facility_id in ('{$session_facility_id}')";
		 } 
	}
    ...
    $count_cond .= " and rt.facility_id not in ('{$outShip_facility}') "; 
 ...
 $smarty->display('supplier_return/supplier_return_goods_request_list.htm');
...
function getReturnRequest($sql_cond,$offset,$size){
	...
	$sql = " select rt.supplier_return_id, rt.created_stamp,rt.last_update_tx_stamp,
		  ...
		  ifnull(sum(-iid.quantity_on_hand_diff),0) as new_out_num ,
		   iid.created_stamp as new_out_time,oir.STATUS as reserve_status
		from romeo.supplier_return_request rt    
		INNER JOIN romeo.facility f ON rt.facility_id = f.FACILITY_ID    
		...
		where  1 {$sql_cond}
		group by rt.supplier_return_id order by rt.created_stamp desc  limit {$offset},{$size} "; 

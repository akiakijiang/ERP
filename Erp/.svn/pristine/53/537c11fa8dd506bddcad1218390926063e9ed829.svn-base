...
admin_priv('brand_goods_maintain_' . $_SESSION['party_id']);
...
if($act == 'save') {
	...
	$result = batch_save_estee_products($codes,$material_numbers,$goods_types);
	if(!$result['success']) {
...
$cond = getCondition();
$goods_list = search_goods($_SESSION['party_id'],$cond);
...
if ($_REQUEST['type'] == '商品导出CSV') {
	...
	$out = $smarty->fetch ( 'oukooext/brand_goods_estee_csv.htm' );
	echo iconv ( "UTF-8", "GB18030", $out );
	exit ();
}
$smarty->display ( 'brand_goods_maintain_estee.html' );
 ...
 * 批量更新
 */
function batch_save_estee_products($codes,$material_numbers,$goods_types) {
	$result = array();
	$len = count($codes);
	// 判断哪些已经存在了
	...
	for($i=0;$i<$len;$i++) {
		...
		if(!empty($exist_codes) && in_array($code,$exist_codes)) {
			$sqls[] = "update ecshop.brand_estee_product set material_number = '{$material_number}',goods_type='{$goods_type}',last_updated_stamp=now() where code='{$code}' and party_id = {$_SESSION['party_id']} limit 1";
		} else {			
			$sqls[] = "insert into ecshop.brand_estee_product (party_id,code,material_number,goods_type,created_stamp,last_updated_stamp) values({$_SESSION['party_id']}, '{$code}','{$material_number}','{$goods_type}',now(),now())";
		}
	}
...
function get_exist_goods($codes) {
	...
	global $db;
	$sql = "select p.code
	        from ecshop.brand_estee_product p
			where p.party_id = {$_SESSION['party_id']} and p.code ".db_create_in($codes);
...
function search_goods($party_id,$cond) {
	...
	$sql = "select gg.code,name as goods_name,ifnull(p.material_number,'') as material_number,p.goods_type,if(p.code is null,1,0) as is_product_empty
			...
			left join ecshop.brand_estee_product p ON p.code = gg.code
			where 
			1 {$cond['tc_cond']}
...
//    var_dump('$tc_goods_list');var_dump($sql);
	...
	$sql = "select g.goods_id as code,g.goods_name,ifnull(p.material_number,'') as material_number,p.goods_type,if(p.code is null,1,0) as is_product_empty 
			...
			left join ecshop.brand_estee_product p ON p.code = g.goods_id
			where 
			1 {$cond['simple_cond']}

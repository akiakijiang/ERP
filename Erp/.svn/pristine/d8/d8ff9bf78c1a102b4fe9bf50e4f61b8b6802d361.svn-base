...
/**
 * 定义 Cache 类
 * 为保证RomeoApi目录的独立性，尽量避免和其他程序共享库文件，所以为它专门写了一个缓存类。
 * 
 * @package cache
 */

/**
 * RomeoApi_Cache 类提供以文件系统来缓存数据的服务
 *
 */
class RomeoApi_Cache
{
    /**
     * 是否允许使用缓存
     *
     ...
     *
     * -  cache_dir_umask: 创建缓存目录时的标志，默认为 0755
     *
     * -  cache_dir: 缓存目录（必须指定）
     *
    ...
    (
        'life_time'         => 3600,//21600,  // 默认6小时
        'serialize'         => true,
        'encoding_filename' => true,
        'cache_dir_depth'   => 1,      // 默认目录级别
    ...
    function __construct(array $default_policy = null)
    {
        ...
        {
            $this->_default_policy['cache_dir'] = ROOT_PATH. 'templates/caches';
        }
    }
    ...
    protected function _path($id, array $policy, $mkdirs = true)
    {
        ...
        if (empty($root_dir))
        {
            throw new Exception('缓存目录没有设置对. 可以在初始化Cache时指定');
        }
        ...
        if ($policy['cache_dir_depth'] > 0 && $mkdirs)
        {
            if (!file_exists(ROOT_PATH . 'templates/caches')) {
            	if(isset($_SERVER['SERVER_NAME'])) {
					@mkdir(ROOT_PATH . 'templates/caches', $policy['cache_dir_umask']);
	        	}
            }
            $hash = md5($filename);
            $root_dir .= 'cache_';

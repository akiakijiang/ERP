...
*/
  ...
  if(address_id){
  Ajax.call(path+'Cart.Controller.php?Action=select_shipping', 'shipping=' + obj.value+'&address_id='+address_id, orderShippingSelectedResponse, 'GET', 'JSON');
  }
}
...
function checkSelect()
{
	...
	if(theFormObj.elements['shipping'].value)
	{
		selectShipping(theFormObj.elements['shipping']);
	}
	
	if(theFormObj.elements['payment'].value)
	{
		selectPayment(theFormObj.elements['payment']);
	}
}
/**
 *
...
var	address_id =	document.getElementById('address_id').value;
if(address_id){
  Ajax.call(path+'Cart.Controller.php?Action=select_payment', 'payment=' + obj.value +'&address_id='+address_id, orderSelectedResponse, 'GET', 'JSON');
	}
}
/* *
 * 团购购物流程 --> 改变配送方式
...
function changeNeedInv()
{
  ...
  Ajax.call(path+'Cart.Controller.php?Action=change_needinv', 'need_idv=' + needInv + '&amp;payee=' + invPayee + '&amp;content=' + invContent, null, 'GET');
}

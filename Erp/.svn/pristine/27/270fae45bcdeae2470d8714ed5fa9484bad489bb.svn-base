<?php
//echo "Express Clearing";

define('IN_ECS', true);
require_once('includes/init.php');
require_once(ROOT_PATH . 'includes/helper/uploader.php');
require_once(ROOT_PATH . 'includes/helper/array.php');
require_once(ROOT_PATH . 'admin/includes/express/cls_abstract_sheet.php');
require_once(ROOT_PATH . 'admin/function.php');
require_once(ROOT_PATH . 'includes/debug/lib_log.php');

//if($_SESSION['admin_name']!='ljni')die('开发中');
...
if($_SESSION['admin_name']!='ljni')admin_priv('express_fee_detail_manage');
$act = $_REQUEST['act'];
...
switch ($act) {
    ...
    case 'export':
        //导入
        if($_SESSION['admin_name']!='ljni')admin_priv('carriage_detail_export');
        $file_name = file_upload();
        $sheet = AbstractSheet::createSheet($shipping_id, $type);
        //excel的读取
        $result = $sheet->read_excel($file_name);
...
$smarty->display("oukooext/express_clearing.htm");
/**

...
if(empty($info)){
	...
	$sql = "
			SELECT
		        ...
		        og.goods_name, og.goods_number, og.customized, og.goods_id, og.style_id, gs.internal_sku, if(gs.barcode is NULL or gs.barcode = '',
	...
	if (!empty( $search_orders )) {
		$in_order_id_str = implode(",",$refs_value_order ['order_id']);
		//新库存：查询每个订单的已入库数
		$sql = "SELECT SUM(IFNULL(iid.QUANTITY_ON_HAND_DIFF,0)) AS new_in_number, og.goods_number AS goods_number,og.order_id,
					...
					WHERE og.order_id in ({$in_order_id_str})
	    ...
	    if(empty($csv)){
	    	// 查询每个订单的收货容器(即上架车)情况
		 	$sql = "
		        SELECT
		            order_id, if(il.inventory_location_id, location_barcode,'已上架') as location_barcode, sum(goods_number_diff) as rec_num
		        FROM
		        ...
		        WHERE
		            order_id in ({$in_order_id_str})  
		        GROUP BY order_id, location_barcode
			";
			$refs_value_count_4 = $refs_count_4 = array ();
			$db->getAllRefBy ( $sql, array ('order_id'), $refs_value_count_4, $refs_count_4 );  
	    }
		...
		foreach ( $search_orders as $key => $order ) {
			...
			if(empty($csv)){
		    	...
		    	$search_orders [$key] ['receive_location_info'] = $refs_count_4 ['order_id'] [$order ['order_id']];
			}
	
			foreach ($receive_infos as $receive_info_key=>$receive_info) {
				if($receive_info['order_id'] == $order['order_id']) {
...
$smarty->display ( 'oukooext/batch_order_statistics.htm' );

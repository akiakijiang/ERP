...
function get_inventory_info($outer_id,$begin_time,$end_time,$offset,$limit,$num_iid=null,$sku_id=null) {
	...
	$sql = "select num_iid,sku_id,outer_id,stock_quantity,facility_reserved_quantity,wait_send_quantity,supplier_return_quantity,(stock_quantity-facility_reserved_quantity-wait_send_quantity) as new_quantity,taobao_quantity,created_time from ecshop.sync_taobao_inventory"
...
function get_inventory_count($outer_id,$begin_time,$end_time) {
	...
	$sql = "select count(1) from ecshop.sync_taobao_inventory" . $and_sql;
...
$num_iid =  isset($_REQUEST['num_iid']) ? trim($_REQUEST['num_iid']) : null;
...
$total, $page_size, $page, 'page', $url = 'taobao_inventory_list.php', null, $extra_params
);

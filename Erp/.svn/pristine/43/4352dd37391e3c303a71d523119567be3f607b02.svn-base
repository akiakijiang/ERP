...
function get_inventory_info($outer_id,$begin_time,$end_time,$offset,$limit) {
	...
	$sql = "select item_code,product_code,outer_id,new_quantity,suning_quantity,facilityReservedQuantity,supplierReturnQuantity,created_stamp from ecshop.sync_suning_inventory" 
...
function get_inventory_count($outer_id,$begin_time,$end_time) {
	...
	$sql = "select count(1) from ecshop.sync_suning_inventory" . $and_sql;
...
$pagination = new Pagination(
    $total, $page_size, $page, 'page', $url = 'suning_inventory_list.php', null, $extra_params
);
...
$smarty->display('oukooext/suning_inventory_list.htm');

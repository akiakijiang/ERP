...
$config = array(
    '采购单信息' => array(
	    ...
	    'order_amount' => '分销商已支付金额',
	    'shipping_fee' => '应付邮费',
	    'order_time' => '采购单创建时间',
	    'distributor_name' => '分销商会员名',
        'postscript' => '采购单留言',
//	    'alipay_account' => '分销商支付宝账号', 
    ...
    switch ($act) {
        ...
        case 'upload' :
            ...
            foreach ($data['采购单信息'] as $row1) {
            	...
            	// 明细
                    ...
                    // 检查套餐金额是否与ERP一致
                    if (strpos($row1['goods_code'], 'TC-') !== false) {
                        $result_flag = distribution_price_check($row1['goods_code'], $row1['goods_price']);
                        if(!$result_flag){
                        	$errGoodsCode[] = $row1['taobao_order_sn'];
                        }
                    }
            ...
            if(!empty($errGoodsCode)){
            	$smarty->assign('message', '导入的淘宝订单：'.implode(', ', $errGoodsCode).' 中套餐金额与ERP系统中维护的金额不一致， 请重新检查。。');
                break;
            }
            // 取得淘宝订单号
        ...
        case 'import' :
            ...
            foreach ($_POST['checked'] as $taobao_order_sn) {
                ...
                $party_id = $_SESSION['party_id'];  // 初始值，party_id需要根据商品所属的party_id来决定
                ...
                else if ($distributor['distributor_id'] == 55) {  // 当当网（电教）
                	$order['pay_id'] = 77;  // 当当直付
                }
                ...
                foreach ($_shipping_list as $shipping_key => $shipping_value) {
                    if (stripos($order['postscript'], $shipping_key) !== false) {
                ...
                if ($order_goods) {
                	// 发到江浙沪、安徽的默认到杭州仓
                	if (in_array($order['province'],array(10,11,12,13))) {
                		$facility_id = 22143846;  // 乐其杭州
                	} else {
                		$facility_id = 3580047;  // 乐其东莞
                	}
                    ...
                    else if ($facility_id==22143846) {
                    	$order['facility_id']=3580047;
                    }
                    else {
                    	$order['facility_id']=22143846;
                    }
                    
                    // 奥普的仓库固定电商服务杭州仓
                    if ($party_id == 65541) {
                        $order['facility_id'] = 22143847;
                    }
                }
...
// 删除动作
    ...
    if ($id) {
        ...
        if ($review && $review['imported'] != 'Y') {
            ...
            header("Location: distribution_order_import.php?info=". urlencode('删除成功')); 
            exit;   
        } else {
            // 订单不存在或者该订单已经导入
            header("Location: distribution_order_import.php?info=". urlencode('该订单不存在或者已导入不能删除')); 
            exit;               
        }
    }
}
...
$pagination = new Pagination(
    $total, $size, $page, 'page', $url = 'distribution_order_import.php', null, $filter
);
...
$smarty->display('distributor/distribution_order_import.htm');

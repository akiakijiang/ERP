...
require_once $shipclient_wsdl = dirname(__FILE__) . "/fedex-common.php";
...
function addressValidation($path_to_wsdl, $data)
{
	...
	catch (SoapFault $exception)
	{
		...
		return array('code' => 1, 'data' => "Code:{$exception->faultcode},String:{$exception->faultstring}");
	}
}
...
function shipClient($path_to_wsdl, $data)
{
	$client = new SoapClient($path_to_wsdl, array(
		'trace' => 1
	)); // Refer to
...
$data['Shipper'] = array(
	'Contact' => array(
		'PersonName' => 'gg guan 1', 
		'CompanyName' => 'gg Sender Company Name', 
		'PhoneNumber' => '1234567890'
	), 
	'Address' => array(
		'StreetLines' => array(//最多2行
			'gg Address Line 1gg Address Line 10123',//每行最多35字符,剩余自动截断
			'gg Address Line 2'//每行最多35字符,剩余自动截断
		), 
		'City' => 'ShangHai', 

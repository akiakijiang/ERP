<?php

/**
 * ECSHOP 文章分类管理程序语言文件
 * ============================================================================
 * 版权所有 (C) 2005-2006 北京亿商互动科技发展有限公司，并保留所有权利。
 * 网站地址: http://www.ecshop.com
 * ----------------------------------------------------------------------------
 * 这是一个免费开源的软件；这意味着您可以在不用于商业目的的前提下对程序代码
 * 进行修改、使用和再发布。
 * ============================================================================
 * @author:     wj      <wjzhangq@126.com>
 * @version:    v2.0
 * ---------------------------------------------
 * $Author: paulgao $
 * $Date: 2007-04-19 18:11:35 +0800 (星期四, 19 四月 2007) $
 * $Id: articlecat.php 8272 2007-04-19 10:11:35Z paulgao $
*/

$_LANG['cat_name'] = '帮助分类名称';
$_LANG['sort_order'] = '排序';
$_LANG['cat_keywords'] = '关键字';
$_LANG['cat_desc'] = '描述';
$_LANG['show_in_footer'] = '是否显示在底部';
$_LANG['show_in_left'] = '是否显示在左侧';
$_LANG['no_helpcat'] = '没有分类';
$_LANG['cat_id'] = '编号';
$_LANG['add_cat'] = "添加分类";
$_LANG['notice_order'] = "调整排序，数值越小排越前面";
$_LANG['button_remove'] = "批量删除";
$_LANG['batch_remove_succeed'] = "批量删除成功";


$_LANG['isopen'] = '显示';
$_LANG['isclose'] = '不显示';
$_LANG['add_article'] = '添加文章';

$_LANG['articlecat_edit'] = '文章分类编辑';

/* 提示信息 */
$_LANG['catname_exist'] = '分类名 %s 已经存在';
$_LANG['back_list'] = '返回分类列表';
$_LANG['continue_add'] = '继续添加新分类';
$_LANG['catadd_succed'] = '已成功添加';
$_LANG['catedit_succed'] = '分类 %s 编辑成功';
$_LANG['back_list'] = '返回分类列表';
$_LANG['continue_add'] = '继续添加分类';
$_LANG['no_catname'] = '请填入分类名';
$_LANG['edit_fail'] = '编辑失败';
$_LANG['enter_int'] = '请输入一个整数';
$_LANG['not_emptycat'] = '分类下还有文章，不允许删除非空分类';

/*帮助信息*/
$_LANG['notice_keywords'] ='关键字为选填项，其目的在于方便外部搜索引擎搜索';
$_LANG['notice_isopen'] ='该文章分类是否显示在前台的主导航栏中。';

/*JS 语言项*/
$_LANG['js_languages']['no_catname'] = '没有输入分类的名称';
$_LANG['js_languages']['remove_confirm'] = '您确定要删除选定的分类吗？';

?>
 ...
 */
class ErpSyncVipshopCommand extends CConsoleCommand
{
   ...
   /**
    ...
    public function actionSyncOrder($appkey=null,$hours=6)
    {
    	...
    	// 远程服务
    	$client=Yii::app()->getComponent('erpsync')->SyncVipshopService;
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncVipshopOrder start \n");
    		...
    		try
    		{
    			...
    			$response=$client->SyncVipshopOrder($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     * @param int hours 订单同步时间距离当前时间的小时数
     */
     public function actionVipshopOrderTransfer($appkey=null,$days=1)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." VipshopOrderTransfer start \n");
			$start = microtime(true);
    		...
    		try
    		{
				...
				$response=$client->VipshopOrderTransfer($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     * @param string $appkey 执行店铺的应用编号
     */
     public function actionSyncVipshopOrderSendDelivery($appkey=null)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncVipshopOrderSendDelivery start \n");
			$start = microtime(true);
    		...
    		try
    		{
				...
				$response=$client->SyncVipshopOrderSendDelivery($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     * @param float hours 
     */
     public function actionSyncVipshopOrderFinished($appkey=null,$hours=6)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncVipshopOrderFinished start \n");
			$start = microtime(true);
    		...
    		{
    			$request=array("applicationKey"=>$taobaoShop['application_key'],"hours"=>$hours);
				$response=$client->SyncVipshopOrderFinished($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
    ...
    protected function getTaobaoShopList()
    {
    	...
    	if(!isset($list))
    	{
    		$sql="select * from taobao_shop_conf where status='OK' and shop_type = 'vipshop'";

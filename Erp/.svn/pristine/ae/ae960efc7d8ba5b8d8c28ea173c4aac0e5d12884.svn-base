...
if($_REQUEST['export']){
		    ...
		    $out = $smarty->fetch('virance_inventory/physical_inventory_out_batchsn_csv.htm');
		    echo iconv("UTF-8", "GB18030", $out);        
}else{
	$smarty->display('virance_inventory/physical_inventory_out_inventory_batchsn.htm');
}
function get_virance_order_info(){
	global $db;
	$condition = getCondition();
	$sql = "select pm.product_id,oi.facility_id,og.rec_id as order_goods_id,og.goods_name,og.goods_number,og.goods_attr, 
		    goods_price, status_id,
			oi.order_sn,
			oi.order_type_id,
...
function deliver_inventory_virance_order_inventory($orderGoodsId,$serialNumber){
	global $db;
	$sql = "select oi.order_id,oi.facility_id,og.status_id,pm.product_id,og.goods_number,
	        og.goods_number,og.goods_price,og.goods_attr,oi.postscript,oi.order_type_id
			from ecshop.ecs_order_goods og
	...
	$batch_sn = "";
	$batch_sql ="";
	if(strpos($row['goods_attr'],"batch_sn") > 0 ){
		$batch_sn = json_decode($row['goods_attr'],true); 
		$batch_sn = $batch_sn['batch_sn'];
		$batch_sql .=" and batch_sn = '{$batch_sn}' "; 
	}
	...
	$sql = "select INVENTORY_ITEM_ACCT_TYPE_ID,INVENTORY_ITEM_TYPE_ID 
			...
			where product_id = '{$productId}' and facility_id = '{$facilityId}' {$batch_sql}
			";
	$row = $db->getRow($sql);
	if(empty($row)){
		die("该商品不存在过, ".$sql);
	...
	$result = createInventoryItemVarianceByProductIdBatchSn(
            ...
            $unitCost, $facilityId,$comment,$orderId,$orderGoodsId,$batch_sn);
    return $result;
}

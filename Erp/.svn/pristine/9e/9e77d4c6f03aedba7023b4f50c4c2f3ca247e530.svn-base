...
function get_inventory_info($outer_id,$begin_time,$end_time,$offset,$limit) {
	...
	$sql = "select ware_id,sku_id,outer_id,new_quantity,jd_quantity,created_time from ecshop.sync_jd_inventory" 
...
function get_inventory_count($outer_id,$begin_time,$end_time) {
	...
	$sql = "select count(1) from ecshop.sync_jd_inventory" . $and_sql;
...
$pagination = new Pagination(
    $total, $page_size, $page, 'page', $url = 'jd_inventory_list.php', null, $extra_params
);
...
$smarty->display('oukooext/jd_inventory_list.htm');

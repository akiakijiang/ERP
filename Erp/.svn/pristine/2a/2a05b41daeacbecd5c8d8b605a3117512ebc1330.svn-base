 ...
 * $Id: shopex46.php 8653 2007-05-17 05:33:17Z paulgao $
 */
...
class shopex46
{
     ...
     * 构造函数
     */
    function shopex46(&$sdb, $sprefix, $sroot, $scharset = 'UTF8', $iconv = NULL)
    {
        $this->sdb = $sdb;
    ...
    function process_cat()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $attr['attr_select']    = ATTR_INPUT;
            $attr['attr_type']      = ATTR_NOT_NEED_SELECT;
            for ($i = 1; $i <= 40; $i++)
            {
                if (trim($row["attr".$i]) != '')
                {
                    $attr['attr_name']  = $this->iconv->Convert($this->scharset, $this->tcharset, $row["attr".$i]);
                    $attr['sort_order'] = $i;
    ...
    function process_brand()
    {
        ...
        {
            $brand = array(
                'brand_name' => $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['brand'])),
                'brand_desc' => '',
            );
    ...
    function process_goods()
    {
        ...
        while ($row = $this->sdb->fetchRow($res))
        {
            ...
            $goods['goods_desc']    = str_replace('pictures/newsimg/', $this->tdocroot . '/images/upload/', $this->iconv->Convert($this->scharset, $this->tcharset, addslashes($row['memo'])));
            $goods['is_real']       = '1';
            ...
            if ($attr_list)
            {
                ...
                for ($i = 1; $i <= 40; $i++)
                {
                    if (trim($row['attr' . $i]) != '')
                    {
                        ...
                        $goods_attr['attr_value'] = trim($this->iconv->Convert($this->scharset, $this->tcharset, $row['attr' . $i]));

 ...
 */
class ErpSyncKdtCommand extends CConsoleCommand
{
   ...
   /**
    ...
    public function actionSyncOrder($appkey=null,$hours=6)
    {
    	...
    	// 远程服务
    	$client=Yii::app()->getComponent('erpsync')->SyncKdtService;
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		try
    		{
    			...
    			$response=$client->SyncKdtOrder($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     * @param int hours 订单同步时间距离当前时间的小时数
     */
     public function actionKdtOrderTransfer($appkey=null,$days=1)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." KdtOrderTransfer start \n");
			$start = microtime(true);
    		...
    		try
    		{
				...
				$response=$client->KdtOrderTransfer($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     * @param string $appkey 执行店铺的应用编号
     */
     public function actionSyncKdtOrderSendDelivery($appkey=null)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncKdtOrderSendDelivery start \n");
			$start = microtime(true);
    		...
    		try
    		{
				...
				$response=$client->SyncKdtOrderSendDelivery($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
     ...
     * @param float hours 
     */
     public function actionSyncKdtOrderFinished($appkey=null,$hours=1)
    {
    	// 不启用商品同步的店铺列表
		$exclude_list=array
		(
    	...
    	foreach($this->getTaobaoShopList() as $taobaoShop)
    	{
    		...
    		echo("[".date('c')."] ".$taobaoShop['nick']." SyncKdtOrderFinished start \n");
			$start = microtime(true);
    		...
    		{
    			$request=array("applicationKey"=>$taobaoShop['application_key'],"hours"=>$hours);
				$response=$client->SyncKdtOrderFinished($request);
    			print_r($response);
    		}
    		catch(Exception $e)
    		{
    ...
    protected function getTaobaoShopList()
    {
    	...
    	if(!isset($list))
    	{
    		$sql="select * from taobao_shop_conf where status='OK' and shop_type = 'koudaitong'";

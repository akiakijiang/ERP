...
class SxjgIndicateCommand extends CConsoleCommand {
		...
		foreach ($orders as $order) {	
			try {
				...
				if ($indicate_type == 'INVENTORY_RETURN') {
					$response=Yii::app()->getComponent('romeo')->IndicateService->createIndicateInventoryReturn(
    ...
    protected function getMaster() {
 	...
 	public function actionInventoryInIndicate(){
		...
		if (!empty($res->return->result)) {
			...
			foreach ($order_list as $key => $item) {
				...
				//指示号处理，+ LQ
				$item->indicateId = 'LQ' . $item->indicateId;
				//入库头指示
				...
				if (!empty($item->indicateDetailList)) {
					...
					foreach ($detail as $goods_item) {
						...
						$txt_info[] = array("RCDTLDNNEW", "{$item->indicateId}", "{$goods_item->sequenceNo}", "NB-FX", "{$goods_item->barcode}", "", 
										"{$goods_item->goodsNumber}", "{$color}", "{$goods_item->styleName}", "", "{$goods_item->sequenceNo}", "分销入库", "{$goods_item->sequenceNo}");
					} 
				}
			}
 	...
 	public function actionInventoryOutIndicate(){
		...
		if (!empty($res->return->result)) {
			...
			foreach ($order_list as $key => $item) {
				...
				$goods_amount = $item->goodsAmount+$item->discountFee;
				$txt_info[] = array("SHHDRDNNEW", "ILC-SH", "{$item->indicateId}", "{$item->indicateId}",
				 "NB-FX", "{$item->taobaoUserId}", "{$goods_amount}", "{$item->shippingFee}", "", "",
				...
				if (!empty($item->indicateDetailList)) {
					...
					foreach ($detail as $goods_item) {
						...
						//样式名称 size
						$txt_info[] = array("SHDTLDNNEW", "{$item->indicateId}", "{$goods_item->sequenceNo}", "{$goods_item->sequenceNo}", "NB-FX", 
							"{$goods_item->barcode}", "{$goods_item->goodsPrice}", "{$goods_item->goodsNumber}","{$color}", "{$goods_item->styleName}", "", "","{$goods_item->sequenceNo}");
					}
				}
			}
			$upload_result = $this->IndicationFile($file_name, $txt_info);
 	...
 	public function actionActual() {
		...
		if ($conn_ftp) {
			global $sxjg_ftp_performance;
			$file_name_list = ftp_nlist($conn_ftp, $sxjg_ftp_performance);
			
			foreach ($file_name_list as $item) {
				$name = basename($item);
				if (strpos($name, ".")) {
					$down_result = $this->downloadFile($conn_ftp, $name, $sxjg_ftp_performance);
					if ($down_result) {
		...
		if (!empty($content) && is_array($content)) {
			...
			foreach ($info as $item) {
				...
				foreach($item['indicate_detail'] as $item_detail){
					$indicateDetailId = (int)$item_detail[2];
					if ($item_detail[2]) {
						$db=Yii::app()->getDb();
						$sql = "select indicate_detail_id from ecshop.ecs_indicate_detail where indicate_id = " . $item_detail[1] . " and sequence_no = " . $item_detail[2] ;
						$query_result = $db->createCommand($sql)->queryAll();
						if ($query_result[0]['indicate_detail_id']) {
							$indicateDetailId = (int)$query_result[0]['indicate_detail_id'];
						}
					}
					
					$c->actualDetailList[] = array("indicateDetailId" => (int)$indicateDetailId, "indicateId" => (int)$item_detail[1], 
						"goodsNumber" => $item_detail[6], "outerDetailSn" => trim($item_detail[7]), "sequenceNo" => (int)$item_detail[2], "goodsType"=> "良品");
				}
 	...
 	protected function InventoryOutOrReturnTxt($content) {
		...
		if (!empty($content) && is_array($content)) {
			...
			foreach ($info as $item) {
				...
				$c->outerSn = $item[9];
				foreach($item['indicate_detail'] as $item_detail){
					$indicateDetailId = (int)$item_detail[7];
					if ($item_detail[7]) {
						$db=Yii::app()->getDb();
						$sql = "select indicate_detail_id from ecshop.ecs_indicate_detail where indicate_id = " . $item_detail[2] . " and sequence_no = " . $item_detail[7] ;
						$query_result = $db->createCommand($sql)->queryAll();
						if ($query_result[0]['indicate_detail_id']) {
							$indicateDetailId = (int)$query_result[0]['indicate_detail_id'];
						}
					}
					$c->actualDetailList[] = array("indicateDetailId" => (int)$indicateDetailId, "indicateId" => (int)$item_detail[2], 
						"goodsType" => trim($item_detail[8]), "goodsNumber" => $item_detail[4], "sequenceNo" => (int)$item_detail[7]);
				}
				$res= $client->createActual(array('actual' => $c));
				if ($res->return->code != "SUCCEED") {
					echo date('c'). " InventoryReturnTxt code:" . $res->return->code ." msg:" . $res->return->msg ." \n";
 	...
 	protected function readTxtFile($ftp_path_name){
		$start = time();
		global $sxjg_ftp_performance;
		$ftp_path_name = ROOT_PATH."../sxjg/".$sxjg_ftp_performance . "/". $ftp_path_name;
		$order_info = array();
 	...
 	protected function IndicationFile ($file_name, $order_list) {
		...
		$write_result = $this->writeFile(ROOT_PATH."../sxjg/indicates_data/".$file_name, $order_list);
		$write_time = time();
		...
		if ($write_result){
			...
			if ($conn_id !== null) {
				$result = $this->uploadFile($conn_id, $file_name, ROOT_PATH."../sxjg/indicates_data/".$file_name);
				ftp_close($conn_id);  
			}
		}
		return $result;
 	...
 	protected function downloadFile ($conn_ftp, $file_name, $file_url) {
		...
		if($buff != -1){
			if(@ftp_chdir($conn_ftp,$file_url)) {            // 假如指定路径正确
				$tempFile = ROOT_PATH."../sxjg/".$file_url."/".$file_name;
			}
 	...
 	protected function uploadFile ($conn_id, $ftp_file_name, $file_name) {
		global $sxjg_ftp_indicates;
		$ftp_dir_file = $sxjg_ftp_indicates."/".$ftp_file_name;
		$result = false;
		$start = time();
 	...
 	protected function erpLoginFtp() {
 	    global $sxjg_ftp_url;
        global $sxjg_ftp_port;
        global $sxjg_ftp_name;
        global $sxjg_ftp_pwd;
        $conn_id = ftp_connect($sxjg_ftp_url, $sxjg_ftp_port) or die("Could not connect");
        echo date('c') . " erpLoginFtp connect" . " \n";
        $login_result = ftp_login($conn_id, $sxjg_ftp_name, $sxjg_ftp_pwd);
        echo date('c') . " erpLoginFtp login" . " \n";
        
 		if(!$conn_id) {
	    	echo "ftp connect failed: host:".$sxjg_ftp_url." port:".$sxjg_ftp_port." user: ". $sxjg_ftp_name ."\n";
 		}
 		if (!$login_result) {
 			$conn_id = null;
 			echo "ftp login failed: host:".$sxjg_ftp_url." port:".$sxjg_ftp_port." user:". $sxjg_ftp_name ."\n";
 		}
 		return $conn_id;
 	}
	...
	private function get_shipping_name ($erp_shipping_name) {
 	...
 	public function actionCheckFile() {
		...
		if ($conn_ftp) {
			global $sxjg_ftp_indicates;
			$file_name_list = ftp_nlist($conn_ftp, $sxjg_ftp_indicates);
			foreach ($file_name_list as $item) {
				$file_name = basename($item);
		...
		if ($file_names) {
			...
			$this->sendMessage("三雄极光未及时处理指示文件【乐其三雄极光监控】");
		}
 	}
 	...
 	function actionErrorLog() {
		...
		$file_names = null;
		if ($conn_ftp) {
			global $sxjg_ftp_error_log;
			$file_name_list = ftp_nlist($conn_ftp, $sxjg_ftp_error_log);
			foreach ($file_name_list as $item) {
				$name = basename($item);
				if (strpos($name, ".")) {
					$down_result = $this->downloadFile($conn_ftp, $name, $sxjg_ftp_error_log);
					if ($down_result) {
						$this->backupFile($conn_ftp, $name);
					}
				}
		...
		if ($file_names) {
			...
			$this->sendMessage("三雄极光有错误日志返回，请及时处理【乐其三雄极光监控】");
		}

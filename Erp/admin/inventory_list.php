...
$type = $_REQUEST ['type'];
if ($type=="导出耗材库存" || $type == '库存清单CSV' || $type == '库存汇总CSV' || $type == '类别汇总CSV') {
...
if($type=="导出耗材库存") {	
	...
	$sql = "		
			SELECT p.name,f.facility_name,pm.product_id,ii.facility_id,
			g.goods_name,
			g.barcode,
			ifnull((select il.location_barcode
			...
			and p.is_leaf='Y' and p.status = 'ok' and p.parent_party_id != '65542'and p.party_id not in(
			'65546','65599','65556','65559'
			)
			WHERE g.is_delete = 0 
	...
	if($condition2) {
		...
		$sql .= " AND " . party_sql ( 'g.goods_party_id' ) . " 
			AND " . facility_sql ( 'ii.facility_id' ) . "   	
			GROUP BY pm.product_id,ii.status_id,ii.facility_id,p.party_id
			ORDER BY g.goods_party_id,ii.facility_id,g.goods_id" ;
	}
...
if ($type == '库存汇总CSV') {
	...
	$smarty->display ( 'oukooext/inventory_list.htm' );
}

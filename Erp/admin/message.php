 ...
 * $Id: message.php 8272 2007-04-19 10:11:35Z paulgao $
*/
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if ($rec_arr[0] == 0)
    {
        ...
        {
            if ($_SESSION['admin_id'] <> $rows['user_id'])
            {
                $sql = "INSERT INTO " .$ecs->table('admin_message'). " (sender_id, receiver_id, send_date, read_date, readed, deleted, title, message) ".
                    "VALUES ('".$_SESSION['admin_id']."', '".$rows['user_id']."', '".date("Y-m-d H:i:s")."', 'N/A', '0', '0', '$_POST[title]', '$_POST[message]')";
                $db->query($sql);
            }
        }
        /*添加链接*/
    ...
    else
    {
        ...
        {
            $sql = "INSERT INTO " .$ecs->table('admin_message'). " (sender_id, receiver_id, send_date, read_date, readed, deleted, title, message) ".
                   "VALUES ('".$_SESSION['admin_id']."', '$id', '".date("Y-m-d H:i:s")."', 'N/A', '0', '0', '$_POST[title]', '$_POST[message]')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$ecs->table('admin_message'). " SET ".
           ...
           "WHERE sender_id = '$msg_arr[sender_id]' AND send_date='$msg_arr[send_date]'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if ($msg_arr['readed'] == 0)
    {
        $msg_arr['read_date'] = date('Y-m-d H:i:s'); //阅读日期为当前日期

        //更新阅读日期和阅读状态
        $sql = "UPDATE " .$ecs->table('admin_message'). " SET ".
               "read_date = '" . $msg_arr['read_date'] . "', ".
               "readed = '1' ".
...
{
    $sql = "INSERT INTO " .$ecs->table('admin_message'). " (sender_id, receiver_id, send_date, read_date, readed, deleted, title, message) ".
           "VALUES ('".$_SESSION['admin_id']."', '$_POST[receiver_id]', '".date("Y-m-d H:i:s")."', 'N/A', '0', '0', '$_POST[title]', '$_POST[message]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "UPDATE ".$ecs->table('admin_message')." SET deleted=1 ".
            " WHERE message_id=$id AND receiver_id='$_SESSION[admin_id]'";
    $db->query($sql);
...
{
    /* 查询条件 */
    $filter['sort_by']    = empty($_REQUEST['sort_by'])    ? 'send_date' : trim($_REQUEST['sort_by']);
    ...
    switch ($filter['msg_type'])
    {
       ...
       case 4:
           ...
           break;
       default:
           $where = " a.receiver_id='".$_SESSION['admin_id']."' AND a.deleted='0'";
    }
    ...
    $sql = "SELECT a.message_id,a.sender_id,a.receiver_id,a.send_date,a.read_date,a.deleted,a.title,a.message,b.user_name".

...
try {
	do {
		...
		if (count ( $goods_ids ) != $check_number || count ( $rec_ids ) != $check_number || count ( $batch_order_sns ) != $check_number || count ( $goods_item_types ) != $check_number || count ( $is_maintain_warrantys ) != $check_number || count ( $goods_barcodes ) != $check_number) {
		...
		for($i = 0; $i < $check_number; $i ++) {
			try {
				...
				// 批次号
				$batch_sns = $_REQUEST ['batch_id_'.$rec_id];
				$batch_sn_numbers = $_REQUEST ['batch_id_num_'.$rec_id];
				...
				for($j=0;$j<$validity_loop;$j++) {
					...
					$info = one_order_purchase_accept_and_location_transaction_V2($order_id,$grouding_barcode,$goods_barcode,$serial_number,
...
$back = add_param_in_url ( "batch_or_in_storageV2.php?", 'info', $back_message . " 耗时" .$cost_time);

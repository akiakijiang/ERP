...
admin_priv('taobao_fenxiao_order_list');
...
//			QLog::log("result=".implode(implode($result)));
			if ($result) {
...
if (isset($_GET['region_id']) && isset($_GET['fenxiao_id'])) {
	$region_id = $_GET['region_id'];
	$fenxiao_id = $_GET['fenxiao_id'];
	$sql = "UPDATE ecshop.sync_taobao_fenxiao_order_info SET receiver_district = (select " .
			" region_name from ecshop.ecs_region  where region_id ='$region_id'),transfer_status = 'NORMAL'" .
			" WHERE fenxiao_id ='$fenxiao_id' ";
 	$abc = $db->query($sql);
 	header("Location:taobao_fenxiao_order_list.php");
}			


if($act == "search"){
	$condition = get_condition();
	$sql = "SELECT supplier_username, distributor_username, tc_order_id, fenxiao_id, created, pay_time, 
					transfer_status, transfer_note, receiver_state, receiver_city, receiver_district
			FROM ecshop.sync_taobao_fenxiao_order_info
			WHERE 1 ".$condition." order by pay_time desc ";
...
$smarty->assign('tc_order_id',trim($_REQUEST['tc_order_id']));
$smarty->assign('buyer_nick',trim($_REQUEST['buyer_nick']));
$smarty->assign('transfer_status',trim($_REQUEST['transfer_status']));
$smarty->assign('order_list',$order_list);
$smarty->display('taobao/taobao_fenxiao_order_list.htm');

function get_condition(){
	
	$condition = "";
	if( trim($_REQUEST['tc_order_id']) != '' ){
		$condition .= " AND tc_order_id='".trim($_REQUEST['tc_order_id'])."'";
	}

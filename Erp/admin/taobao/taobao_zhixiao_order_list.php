...
if(!in_array($_SESSION['admin_name'],array(
	'mjzhou','zjli','wlchen','hzhang1','qyyao','stsun','jldai'
))){
	$bird_authority = "no_authority";
}else{
	$bird_authority = "have_authority";
}
$batch_retro_order_msg="";
...
//			QLog::log("replace_city = ".$replace_city);
		...
		case 'retro_order':
			...
			$sql = "update ecshop.sync_taobao_order_info set transfer_status='NORMAL',transfer_note = concat('转化失败，手动回退状态，原错误类型:' ,transfer_note )  where tid = '{$out_biz_code}'";
			//var_dump( $sql );
...
if (isset($_GET['region_id']) && isset($_GET['tid'])) {
	...
	$tid = $_GET['tid'];
	$sql = "UPDATE ecshop.sync_taobao_order_info SET receiver_district = (select " .
			" region_name from ecshop.ecs_region  where region_id ='$region_id'),transfer_status = 'NORMAL'" .
			" WHERE tid ='$tid' ";
 	$abc = $db->query($sql);
 	header("Location:taobao_zhixiao_order_list.php");
}			

if($act == "search"){
	$condition = get_condition();
	$sql = "SELECT title, tid, created, pay_time, transfer_status, transfer_note, receiver_state, 
				receiver_city, receiver_district, IF(SHIP_CODE = 'OUT_SHIP','外包发货','自己发货') AS ship_type 
			FROM ecshop.sync_taobao_order_info
			WHERE status not in ('TRADE_CLOSED_BY_TAOBAO','TRADE_CLOSED')".$condition." order by pay_time desc";
	...
	foreach($result_list as $index=>$result) {
		if($result['transfer_status']=='ERROR') {
			...
			if($result['receiver_state']!=null && $result['receiver_city']!=null && $result['receiver_district']==null) {
				$advice = '请查询并选择区信息';
			} else {
				$advice = '请联系erp';
			}
...
elseif($act == "batch_retro_order"){  //订单回退状态
	$orders_list= $_POST['checked'];
	$order_count = count($orders_list);
	for($i=0;$i<$order_count;$i++){
		$sql = "update ecshop.sync_taobao_order_info set transfer_status='NORMAL',transfer_note = concat('转化失败，手动回退状态，原错误类型:' ,transfer_note )  where tid = '{$orders_list[$i]}'";
		$result=$GLOBALS['db']->query($sql);
	}
	$batch_retro_order_msg = '批量回退状态成功'.'共回退'.$order_count.'个订单状态为NORMAL';
...
$smarty->display('taobao/taobao_zhixiao_order_list.htm');
...
function get_condition(){
	...
	if( trim($_REQUEST['order_id']) != '' ){
		$condition .= " AND tid='".trim($_REQUEST['order_id'])."'";
	}
	if( trim($_REQUEST['buyer_nick']) != '' ){
		$condition .= " AND buyer_nick = '".trim($_REQUEST['buyer_nick'])."'";
	}
	if( trim($_REQUEST['transfer_status']) != 'ALL' ){
		$condition .= " AND transfer_status = '".trim($_REQUEST['transfer_status'])."'";
	}
	if($startTime && $endTime){
		$condition .= "and pay_time > '".trim($_REQUEST['startTime'])."' and pay_time <= '".trim($_REQUEST['endTime'])."'";
	}

 ...
 * 收获入库
 * 
 * @author last modified by XiangYang 2009/4/29
 */
...
if ($label == 'common_in_storage') {
	...
	if ( ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['act']=='search') || $csv)
	{
	    ...
	    (
	        ...
	        'batch_order_sn'       => $_REQUEST['batch_order_sn'],                                                     // 采购单号
	...
	$sql = "
		SELECT
	        ...
	        bo.order_type, bo.provider_id, bo.purchaser, o.facility_id, ep.provider_name,bo.batch_order_sn
		FROM 
...
$smarty->display('oukooext/in_storageV2.htm');
...
function get_condition()
{
    ...
    $batch_order_sn = trim($_SESSION['in_storage']['batch_order_sn']);
    ...
    if ($batch_order_sn != '')
    {
        $condition .= " AND bo.batch_order_sn LIKE '%{$batch_order_sn}%'";
    }
    if ($goods_name != '')
    {

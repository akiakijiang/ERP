...
$monitor_header = new MonitorHeader("销售订单取消订单监控页",array('order_id', 'order_sn'));
...
function GenerateOrderInfo($order_id){
	...
	$sql = "SELECT order_id, order_status, pay_status, bonus_id
	...
	//romeo.order_inv_reserved
	$sql = "SELECT indicate_status 
			from ecshop.ecs_indicate
			where order_id = {$order_id}";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL(
		'ecshop.ecs_indicate', $sql, 'indicate_id');
	$data_for_generate[] = $result['monitor_info'];

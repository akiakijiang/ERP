...
$monitor_header = new MonitorHeader("批次采购订单信息监控页",array('batch_order_id', 'batch_order_sn'));
$smarty->assign('monitor_header', $monitor_header);

if(empty($_REQUEST['batch_order_id']) && empty($_REQUEST['batch_order_sn'])){
	$smarty->assign('msg', '请输入batch_order_id或batch_order_sn');
}else
{
	if(!empty($_REQUEST['batch_order_id'])){
		$batch_order_id = $_REQUEST['batch_order_id'];
	}else{
		$sql = "SELECT batch_order_id from ecshop.ecs_batch_order_info where batch_order_sn = '{$_REQUEST['batch_order_sn']}'";
		$batch_order_id = $db->getOne($sql);
	}
	$smarty->assign('monitor_data', GeneratePurchaseOrderInfo($batch_order_id));
}
$smarty->display('SinriTest/common_monitor.htm');


function GeneratePurchaseOrderInfo($batch_order_id){
	global $db;
	...
	$sql = "SELECT eoi.order_id, order_sn, party_id, facility_id, user_id, order_time, order_status, pay_status, order_type_id, currency,
				is_purchase_paid,  purchase_paid_type, purchase_paid_amount, purchase_paid_time, cancel_time,over_time,purchaser, purchase_invoice, 
	...
	//价格跟踪系统
	/*$goods_ids_str = $order_goods_result['query_info']['goods_id'];
	$sql = "SELECT * from PRICE_TRACKER.PROVIDER_PRICE where provider_price_id in (SELECT max(provider_price_id)
			from PRICE_TRACKER.PROVIDER_PRICE
			where GOODS_ID in ({$goods_ids_str})
			group by goods_id, goods_style_id)";
	$result = GetTableMonitorInfoAndAdditionalQueryInfoFromSQL(
		'价格跟踪表[PRICE_TRACKER.PROVIDER_PRICE]', $sql, 'PROVIDER_PRICE_ID');
	$purchase_info_for_generate[] = $result['monitor_info'];*/

...
if(!empty($_REQUEST['order_sn'])) {
	$cond .= " oi.order_sn='".$_REQUEST['order_sn']."' ";
}
...
{
	$sql = "SELECT oi.order_sn,oi.order_id,oi.facility_id,group_concat(pm.product_id) as product_ids
...
//    pp($sql);
	...
	if(empty($result)){
		...
		$smarty->assign('monitor_data', GenerateMonitorInfo($result['facility_id'], $result['order_id'], $result['product_ids']));
	}
}
$smarty->display('SinriTest/common_monitor.htm');


function GenerateMonitorInfo($facility_id, $order_id, $product_ids){
	global $db;
	$product_ids = explode(',',$product_ids);
	
	//订单相关
	//ecshop.ecs_order_info
	$sql = "SELECT order_id,order_sn,order_status,shipping_status,pay_status,shipping_id,shipping_name,facility_id

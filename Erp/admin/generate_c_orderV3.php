...
if($act == 'in_stock'){
	...
	$sql = "
		...
		and eoi.order_type_id = 'PURCHASE'
	...
	$sql = "
		...
		where eoi.order_type_id = 'PURCHASE'
	...
	if($pre_pencentage != $update_pencentage){
		if($batch_order_sn && $update_pencentage){
			$sql = "select oi.order_sn,og.order_id,og.goods_id,og.style_id,boi.provider_id,boi.order_type,ii.UNIT_COST,ivh.unit_cost as init_cost
				...
				where boi.batch_order_sn = '{$batch_order_sn}' GROUP BY og.rec_id
...
//  目前只有香港平世的才会是非人民币的币种
$currencies = (in_array($_SESSION['party_id'],array('65566','65630','65543','65638'))) ? get_currency_style() : (($_SESSION['party_id']=='65536')?array('HKD' => '港币', 'USD' => '美元', 'RMB' => '人民币'):array('RMB' => '人民币'));
...
$sql = "
    ...
    FROM 
        ...
        left join romeo.purchase_order_info poi on og.rec_id = poi.order_goods_id
...
$search_orders = $db->getAllRefby($sql, array('rec_id'), $ref_fields, $ref_orders, false);
if ($search_orders) {
    $in_order_ids = db_create_in($ref_fields['rec_id']);
    
    //统计新库存入库数量
    $sql = " select oi.order_id,ii.unit_cost as purchase_paid_amount,og.rec_id,
		     ...
		     left join romeo.inventory_item_detail iid ON iid.order_id=convert(og.order_id using utf8)   and  iid.order_goods_id=convert(og.rec_id using utf8) 
		     left join romeo.inventory_item ii ON iid.inventory_item_id = ii.inventory_item_id
		     where 
		     og.rec_id {$in_order_ids} and ii.status_id = 'INV_STTS_AVAILABLE'
		     group by og.rec_id ";  
     $refs_temp_new = $refs_count_new = array();
     $db->getAllRefby($sql, array('rec_id'), $refs_temp_new, $refs_count_new, true);
    // 判断废除
	$sql = "select
                bom.order_id, 1 as canceled
        from {$ecs->table('batch_order_mapping')} as bom inner join ecshop.ecs_order_goods og on og.order_id = bom.order_id
            where bom.is_cancelled = 'Y' and og.rec_id {$in_order_ids} group by bom.order_id";
    $refs_temp2 = $refs_canceled = array();
    $db->getAllRefby($sql, array('order_id'), $refs_temp2, $refs_canceled, false);
    // 组装订单数据
    foreach($ref_orders['rec_id'] as $order_id => $group) {
        $order = & $ref_orders['rec_id'][$order_id][0];
        $order['new_in_count'] = $refs_count_new['rec_id'][$order_id][0]['new_in_num'] ? $refs_count_new['rec_id'][$order_id][0]['new_in_num'] : 0;
        $order['purchase_paid_amount'] = $refs_count_new['rec_id'][$order_id][0]['purchase_paid_amount'] ? $refs_count_new['rec_id'][$order_id][0]['purchase_paid_amount'] : $order['purchase_unit_price'];
...
if ($csv == "1" ) {
    ...
    $smarty->display('oukooext/generate_c_orderV3.html');
}

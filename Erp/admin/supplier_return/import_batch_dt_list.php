<?php
	...
	$config = array('供应商批次号退货申请（-gt）清单'  =>
				array('batch_sn'=>'批次号',
				      ...
				      'order_type_id'=>'退货订单类型(调拨订单-0)',
					  'chequeNo'=>'支票号',
				      'remark'=>'备注',
				      'currency'=>'币种',
				      'facility_id_in' =>'入货仓库编码',
				      'arrive_time'=>'到货时间'
				));
	...
	if($final['message'] == ""){
		foreach ($rowset as $row) {
        	...
        	$obj_data->serial_number = $serial_str;
		 	...
		 	$sql = "
			   SELECT pm.ecs_goods_id,pm.ecs_style_id
                  from romeo.product_mapping pm 
	                WHERE pm.product_id= '{$row['product_id']}'
			 ";
			 
			$goods_style = $db -> getRow($sql);
		 	$obj_data->ret_goods_id = $goods_style['ecs_goods_id'];
		 	$obj_data->ret_style_id = $goods_style['ecs_style_id'];;
		 	...
		 	$obj_data->arrive_time = $row['arrive_time'] ;
//			$final['show'] .= "【".apply_ret_req($obj_data)."】  "; 
	        ...
	        $obj_data->ret_facility_id_dt = $row['facility_id_in'] ;
	        $obj_data->remark = $row['remark'].$supplier_request_id ;//备注 
	        //下面用于不同供应商的产生不同的批次
   ...
   function check_batch_sn_import_data($rowset,$supplier_order_type_r){
		...
		foreach ($rowset as $row) {							
			...
			if( empty($row['facility_id_in']) ){
				 $display_error[$num] .="入货仓库编码　不能为空";
			}
		    ...
		    if(!preg_match($reg, $row['ret_amount'])) {
		 	...
		 	$status_map_r = array('全新' => 'INV_STTS_AVAILABLE', '二手' => 'INV_STTS_USED', '次品' => 'INV_STTS_DEFECTIVE') ;
		 	$sql = "select AVAILABLE_TO_RESERVED from romeo.inventory_summary where  product_id = '{$row['product_id']}' and facility_id = '{$row['facility_id']}' 

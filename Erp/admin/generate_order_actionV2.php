...
if($result['err_no'] != '0')
    ...
    $back = $_REQUEST['back'] ? $_REQUEST['back']:'generate_c_orderV2.php';
...
function destroy_c($action_time)
{
    ...
    $facility_type=OMSSERVER_API_DELEGATE::getErpOrderFacilityType($order_id);
    if($facility_type=='LEQEE_WMS'){
        $omsserver_api=new OMSSERVER_API_DELEGATE();
        $order_sn=OMSSERVER_API_DELEGATE::getErpOrderSnWithId($order_id);
        $omsserver_api_cancel_done=$omsserver_api->order_cancel($order_sn);
        if(!$omsserver_api_cancel_done){
            $result['err_no'] = -1;
            $result['message'] = "采购订单去OMSSERVER撤回失败，请重新废除";
            return $result;
        }
    }
    	 ...
    	 $omsserver_api=new OMSSERVER_API_DELEGATE();
        $order_sn=OMSSERVER_API_DELEGATE::getErpOrderSnWithId($order_id);
        $omsserver_api_cancel_done=$omsserver_api->order_cainiao_cancel($order_sn);
        if(!$omsserver_api_cancel_done){
            $result['err_no'] = -1;
            $result['message'] = "采购订单去OMSSERVERCAINIAO撤回失败，请重新废除".$order_sn;
            return $result;
        }
    }
...
function c($action_time)
{
	...
	do {
        ...
        $sql = "INSERT INTO {$ecs->table('batch_order_info')}
                    ...
                    VALUES('{$batch_order_sn}', '".$party_id."', '{$facility_id}', '{$_REQUEST['arrive_time_']}' ,NOW(),
    ...
    foreach ($_POST['goods_id'] as $goods_id_key => $goods_id) {
        ...
        do {
            ...
            $sql = "INSERT INTO {$ecs->table('order_info')}
                    ...
                    '".$party_id."', '{$facility_id}', '{$_REQUEST['currency']}', 'PURCHASE')";
            $db->query($sql, 'SILENT');
            $error_no = $db->errno();
            if ($error_no > 0 && $error_no != 1062) {
                $db->rollback();

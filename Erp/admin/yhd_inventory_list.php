 ...
 * @param $outer_id
 */
function get_inventory_info($outer_id,$begin_time,$end_time,$offset,$limit,$ware_id=null,$sku_id=null) {
	global $db;
	...
	$sql = "select ware_id,sku_id,outer_id,stock_quantity,wait_send_quantity, new_quantity,yhd_quantity,created_time from ecshop.sync_yhd_inventory"
...
function get_inventory_count($outer_id,$begin_time,$end_time) {
	...
	$sql = "select count(1) from ecshop.sync_yhd_inventory" . $and_sql;
	return intval($db->getOne($sql));
}


// 处理请求
$outerId = isset($_REQUEST['outerId']) ? $_REQUEST['outerId'] :(isset($_REQUEST['filter_outerId']) ? $_REQUEST['filter_outerId'] : null);
$ware_id =  isset($_REQUEST['ware_id']) ? trim($_REQUEST['num_iid']) : null;
$sku_id = isset($_REQUEST['sku_id']) ? trim($_REQUEST['sku_id']) : null;
...
$total, $page_size, $page, 'page', $url = 'yhd_inventory_list.php', null, $extra_params
);

// 取得库存同步情况列表
$inventory_info = get_inventory_info($outerId,$start,$end,$offset,$limit,$ware_id,$sku_id);
...
$smarty->display('oukooext/yhd_inventory_list.htm');

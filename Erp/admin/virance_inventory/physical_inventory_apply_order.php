...
//    if(count($comment) == 0) die('请输入调整单的原因');
...
//    if (!$facility_id) die('请选择仓库');
    ...
    foreach ($_POST['goods_id'] as $key => $goods_id) {
        ...
        $facilityIdOne = trim($_POST['facility_id'][$key]); 
...
//					 inner join romeo.product_mapping pm on ii.product_id = pm.product_id
//				where pm.ecs_style_id = '{$styleId}' and pm.ecs_goods_id = '{$goodsId}' and ii.facility_id = '{$facility_id}' 
//					 and ii.status_id = '{$statusId}' order by ii.unit_cost asc";
		...
		if($quantityOnHandVar > 0){
	    	$sql = "select ii.unit_cost
					...
					where pm.ecs_style_id = '{$styleId}' and pm.ecs_goods_id = '{$goodsId}' and ii.facility_id = '{$facilityIdOne}' 
						 and ii.status_id = '{$statusId}' order by ii.created_stamp desc ";
			$purchase_paid_amount = $db->getOne($sql);
		}else{
			$sql = "select ii.unit_cost
						from romeo.inventory_item ii
						 inner join romeo.product_mapping pm on ii.product_id = pm.product_id
					where pm.ecs_style_id = '{$styleId}' and pm.ecs_goods_id = '{$goodsId}' and ii.facility_id = '{$facilityIdOne}' 
						 and ii.status_id = '{$statusId}' and ii.quantity_on_hand_total > 0
...
$smarty->assign('facility_id',$_REQUEST['facility_id']);
$smarty->assign('goods_select_barcode',$_REQUEST['goods_select_barcode']);
$smarty->assign('goods_select_name',$_REQUEST['goods_select_name']);
$smarty->display('virance_inventory/physical_inventory_apply_order.htm');
...
function get_virance_order_info(){
	...
	$barcode = trim($_REQUEST['goods_select_barcode']);
	if($barcode != ''){
		$condition .=" AND IFNULL(gs.barcode,g.barcode) = '{$barcode}' ";
	}
	$name = trim($_REQUEST['goods_select_name']);
	if($name != ''){
		$condition .=" AND og.goods_name like '%{$name}%' ";
	}
	$sql = 
	   "
	   select oi.facility_id,og.rec_id,og.goods_name,og.goods_number, ifnull(ii.unit_cost, og.goods_price) unit_cost, og.status_id,
			  ...
			  f.facility_name,oi.order_time,iid.created_stamp as out_time,oir.status as reserve_status

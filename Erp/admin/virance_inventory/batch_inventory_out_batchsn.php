...
$smarty->display('virance_inventory/batch_inventory_out_batchsn.htm');
...
function get_virance_order_info($tpl){
	...
	if($rowset){
		foreach($rowset as $key=>$row){
			...
			$input_batch_sn = trim($row['batch_sn']); 
			...
			$sql = "select pm.product_id,oi.facility_id,og.rec_id as order_goods_id,og.goods_name,og.goods_number,
				og.goods_attr, goods_price, status_id,
				oi.order_sn,
				oi.order_type_id,
	...
	if(trim($batch_sn) != $input_batch_sn ){
		$message .= $common." 批次号应为{$batch_sn}<br/>";
		continue;
	}
...
function deliver_inventory_virance_order_inventory($orderGoodsId,$serialNumber){
	global $db;
	$sql = "select oi.order_id,oi.facility_id,og.status_id,pm.product_id,
	        og.goods_attr,og.goods_number,og.goods_price,oi.postscript,oi.order_type_id
			from ecshop.ecs_order_goods og

...
admin_priv('brand_integration_monitor_'.$_SESSION['party_id']);
...
//同步状态
$sql = "SELECT sync_status, count(*) as count from ecshop.brand_estee_header where header_status = 'OK' and party_id = {$_SESSION['party_id']} GROUP BY sync_status";
$sync_status_list = $db->getAll($sql);
$sync_status_mapping = array("INIT" => "未开始同步", 'Doing'=>'发送中','Finish'=> '完成同步','ConnectionErr' => '连接超时', 'WrongData' => '生成数据错误', 'WrongXML' => '传输数据格式错误','ProxyException' => 'Estee代理错误');
...
if(!empty($data_list['order_id'])){
	foreach ($data_list['order_id'] as $order_id => $PO_list) {
		...
		$order_info['estee_order_id'] = $PO_list[0]['estee_order_id'];
...
$smarty->assign('party_name', $db->getOne("select name from romeo.party where party_id = {$_SESSION['party_id']}"));
$smarty->display('brand_integration_monitor_estee.html');
die();


function get_sql(){
	if(!isset($_REQUEST['search_type']) || $_REQUEST['search_type'] == 'NEED_SYNC_BUT_NOT'){
		$sql = "SELECT eoi.order_id, order_sn, eoi.taobao_order_sn, boeor.remark, boeor.remarker, boo.estee_order_id, order_time, stoi.pay_time as pricing_time, from_unixtime(eoi.shipping_time) as shipping_time,
					null as estee_header_id, null as document_id, null as created_stamp, null as sync_status, null as sync_time,
					null as header_status, null as order_type
					FROM ecshop.ecs_order_info eoi
					LEFT JOIN ecshop.brand_estee_erp_order_remark boeor on eoi.order_id = boeor.order_id
					LEFT JOIN ecshop.brand_estee_order boo ON boo.order_id = eoi.order_id
					LEFT JOIN ecshop.sync_taobao_order_info stoi ON stoi.tid = eoi.taobao_order_sn
					WHERE eoi.order_type_id = 'SALE' AND eoi.party_id = {$_SESSION['party_id']} AND (eoi.taobao_order_sn <> '' AND eoi.taobao_order_sn is not null) 
						AND eoi.order_status = 1 AND eoi.shipping_status in (1, 2, 3) AND boo.estee_order_id is null
						AND eoi.facility_id in ('137059426') AND eoi.order_time > DATE_SUB(curdate(),INTERVAL 1 month) 
				UNION
				SELECT oi.order_id, oi.order_sn, oi.taobao_order_sn, boeor.remark, boeor.remarker, boo.estee_order_id, oi.order_time, null as pricing_time, from_unixtime(oi.shipping_time) as shipping_time,
								null as estee_header_id, null as document_id, null as created_stamp, null as sync_status, null as sync_time,
								null as header_status, null as order_type
				FROM ecshop.ecs_order_info oi
					LEFT JOIN ecshop.brand_estee_erp_order_remark boeor on oi.order_id = boeor.order_id
								INNER JOIN romeo.refund r ON convert(oi.order_id using utf8) = r.order_id
								LEFT JOIN ecshop.brand_estee_order boo ON boo.order_id = oi.order_id
								INNER JOIN ecshop.order_relation orl ON oi.order_id = orl.order_id
								INNER JOIN ecshop.brand_estee_order rboo ON rboo.order_id = orl.root_order_id
								INNER JOIN ecshop.ecs_order_info roi ON roi.order_id = orl.root_order_id
				WHERE oi.order_type_id = 'RMA_RETURN' AND oi.party_id = {$_SESSION['party_id']} AND r.status IN ('RFND_STTS_CHECK_OK','RFND_STTS_EXECUTED') 
								  AND (roi.taobao_order_sn <> '' AND roi.taobao_order_sn is not null)  AND boo.estee_order_id is null
								  AND oi.facility_id in ('137059426') AND roi.order_time > DATE_SUB(curdate(),INTERVAL 1 month) 
				UNION
				SELECT toi.order_id, toi.order_sn, toi.taobao_order_sn, boeor.remark, boeor.remarker, null as estee_order_id, toi.order_time, null as pricing_time, from_unixtime(toi.shipping_time) as shipping_time, 
								null as estee_header_id, null as document_id, null as created_stamp, null as sync_status, null as sync_time,
								null as header_status, null as order_type
				FROM ecshop.ecs_order_info hoi
					LEFT JOIN ecshop.brand_estee_erp_order_remark boeor on hoi.order_id = boeor.order_id
							  ...
							  INNER JOIN ecshop.order_relation orl ON hoi.order_id = orl.order_id
								INNER JOIN ecshop.brand_estee_order rboo ON rboo.order_id = orl.root_order_id
								INNER JOIN ecshop.ecs_order_info roi ON roi.order_id = orl.root_order_id
							  LEFT JOIN ecshop.brand_estee_order tboo ON tboo.order_id = toi.order_id
							  LEFT JOIN romeo.refund r ON convert(hoi.order_id using utf8) = r.order_id
				WHERE hoi.order_type_id = 'RMA_EXCHANGE' AND hoi.party_id = {$_SESSION['party_id']} AND hoi.order_status = 1 AND hoi.shipping_status in (1, 2, 3)
								    AND (roi.taobao_order_sn <> '' AND roi.taobao_order_sn is not null) AND tboo.estee_order_id is null
								    AND hoi.facility_id in ('137059426') AND roi.order_time > DATE_SUB(curdate(),INTERVAL 1 month) 
				UNION
				SELECT oi.order_id, oi.order_sn, root_eoi.taobao_order_sn, boeor.remark, boeor.remarker, null as estee_order_id, oi.order_time, stoi.pay_time as pricing_time, from_unixtime(oi.shipping_time) as shipping_time, 
								null as estee_header_id, null as document_id, null as created_stamp, null as sync_status, null as sync_time,
								null as header_status, null as order_type
				FROM ecshop.ecs_order_info oi
					LEFT JOIN ecshop.brand_estee_erp_order_remark boeor on oi.order_id = boeor.order_id
								...
								INNER JOIN ecshop.brand_estee_order tboo ON s.back_order_id = tboo.order_id
				...
				WHERE oi.order_type_id = 'RMA_EXCHANGE' AND oi.party_id = {$_SESSION['party_id']} AND (root_eoi.taobao_order_sn <> '' AND root_eoi.taobao_order_sn is not null) 
									AND oi.order_status = 1 AND oi.shipping_status in (1, 2, 3) AND boo.estee_order_id is null
									AND oi.facility_id in ('137059426') AND oi.order_time > DATE_SUB(curdate(),INTERVAL 1 month) ";

	}else{
		$sql = "SELECT eoi.order_id, order_sn, eoi.taobao_order_sn, boeor.remark, boeor.remarker, boo.estee_order_id, order_time, stoi.pay_time as pricing_time, from_unixtime(eoi.shipping_time) as shipping_time,
					boh.estee_header_id, boh.document_id, boh.created_stamp, boh.sync_status, if(boh.sync_status='INIT', '', boh.last_updated_stamp) as sync_time,
					boh.header_status, boh.order_type
				from ecshop.ecs_order_info eoi
					LEFT JOIN ecshop.brand_estee_erp_order_remark boeor on eoi.order_id = boeor.order_id
					LEFT JOIN ecshop.sync_taobao_order_info stoi ON stoi.tid = eoi.taobao_order_sn
					LEFT JOIN ecshop.brand_estee_order boo ON eoi.order_id = boo.order_id
					LEFT JOIN ecshop.brand_estee_header boh on eoi.order_id = boh.order_id
				where eoi.party_id = {$_SESSION['party_id']}  AND eoi.facility_id in ('137059426') " . get_condition() . " order by eoi.order_time limit 100";
	}
	return $sql;
} 
...
function get_condition() {
	...
	if($po_status && $po_status != 'ALL') {
		...
		$cond .= " and (boh.estee_header_id is null or boh.header_status = 'OK') ";
	}
	...
	if(isset($_REQUEST["search_erp_to_po_status"])){
		if($erp_to_po_status === "ALL"){
			...
			$cond .= " and boh.estee_header_id is not null ";
		}else{
			$cond .= " and boh.estee_header_id is null ";
		}
	}
